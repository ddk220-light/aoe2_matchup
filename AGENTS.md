...
```bash
PORT=5002 python apps/website/app.py                 # local dev server
...
Production start (Railway): `railway.json` runs `gunicorn app:app --workers 2 --timeout 300` from `apps/website/` (the Procfile is a fallback). Railway installs `apps/website/requirements.txt` (Flask, numpy, Pillow, imageio-ffmpeg, pinned `aoc-mgz` fork); top-level `requirements.txt` is minimal.

## Architecture — quick map

Full detail: `docs/architecture/README.md`. The short version:

1. **`aoe2x/extract/`** — parses the dat via genieutils-py into 8 JSONs (units, technologies, tech_ages, civilizations, armor_classes, effects, tech_effects, civ_tech_trees).
2. **`aoe2x/dbgen/generate_reference.py`** — applies tech effects/civ bonuses per civ into `aoe2_reference.db` with a full audit trail (`ref_stat_chain` records every step). Hardcoded combat properties layer on top from `aoe2x/dbgen/config_combat.py` (later wins): `extracted dat → COMBAT_PROPERTIES → UNIQUE_COMBAT_PROPERTIES → CIV_COMBAT_PROPERTIES`.
3. **`aoe2x/dbgen/generate_main_db.py`** — flattens into `aoe2_units.db` `unit_stats` (100 columns). Legacy: the app reads `aoe2_reference.db` instead.
4. **`apps/website/`** — Flask (`app.py`, 24 routes + 7-route replay blueprint) + shared static assets (`static/js/*.js`, `static/css/*.css` — **not** inlined in templates anymore).
...
- Skills in `.Codex/skills/`: `webapp-architecture`, `running-simulations`, `researching-game-data`, `aoe2onlinereference`.
- Agents in `.Codex/agents/`: `unit-stats-analyzer` (full stat/tech breakdown for a unit×civ), `webapp-test-runner` (spins up local server + runs validations).
...
2. **Never `git push origin main` from a Codex session unless explicitly asked.** A push to `main` ships to production. If a fix needs to ship, push to `staging`, ask the user to verify on the staging URL, then ask before promoting.
...
6. **Matchup DBs live OUTSIDE the repo** (`D:/AI/matchup_baseline_<build>.db`, 200+ MB) — never commit them. (The old 3.9 MB Armenians-only `apps/website/matchup_db.db` stub was removed 2026-06-11; derive scripts take `--matchup-db` and pre-flight-reject partial DBs.)
7. **Committed data artifacts ARE the deployment mechanism** — both environments serve whatever's on their branch: `data/golden/aoe2_reference.db`, `aoe2_units.db`, `derived_data.db`, `pool_scores.db`, `patches.db`, `civ_power_units/*.json`, `civ_top_units.json`, `train_times.json`. Regenerate → commit on `staging` → smoke-test → promote.
