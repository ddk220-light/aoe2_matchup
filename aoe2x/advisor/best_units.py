...
def _build_unit_entry(row, civ_name, conn, db_age, reference_techs, techs_by_slug, effects_by_slug, line_counts=None, score_type="", ease_by_slug=None):
    ...
    Percentiles come from the published rank within the unit line, so civ pages
    and the rankings table share the same source and ordering.
    """
    slug = row["unit_slug"]
...
def compute_civ_power_units(build_number=None):
    ...
    conn = _get_db()
    rc = conn.cursor()
    derived_conn = _get_rankings_derived_db()
...
def get_matchup_recommendations(civ_a, civ_b, age="imperial"):
    ...
    derived_conn = _get_advisor_derived_db()
    drc = derived_conn.cursor()
