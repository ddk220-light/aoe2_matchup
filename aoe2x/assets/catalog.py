...
ANIM_SHEETS_PATH = str(WEBAPP_DIR / "static" / "data" / "anim_sheets.json")
...
def load_anim_sheets() -> dict:
    """slug -> {frames, fw, fh, dur} for attack sprite-sheets in the bucket
    (sheets/<slug>.webp), used by the canvas battle for frame playback."""
    try:
        with open(ANIM_SHEETS_PATH) as f:
            return json.load(f)
    except FileNotFoundError:
        return {}
...
def build_catalog(manifest: dict, icon_names: list, asset_base: str, build: str,
                  anim_slugs=(), anim_sheets=None) -> dict:
    sprites = {}
    ...
    if asset_base:
        have = set(anim_slugs)
        sheet_meta = anim_sheets or {}
        for name, e in manifest.items():
            slug = e["slug"]
            if slug in have:
                anims[name] = f"{asset_base}/anim/{slug}.webp"
            m = sheet_meta.get(slug)
            if m:
...
def synthesize_local(asset_base: str = "", build: str = "local") -> dict:
    ...
    return build_catalog(manifest, icon_names, asset_base, build,
                         anim_slugs=load_anim_slugs(), anim_sheets=load_anim_sheets())
