...
def main():
    ...
    if not args.dry_run and not (config.assets_enabled() and config.database_url()):
        raise SystemExit("BUCKET_* + DATABASE_URL required (or use --dry-run)")
    ...
    # 2) catalog -> Postgres. URLs point at the /assets broker route (same-origin).
