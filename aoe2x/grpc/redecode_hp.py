...
sys.path.insert(0, os.path.dirname(os.path.abspath(__file__)))   # self-contained
import decode_state_v2 as D          # noqa: E402
import cade_api_pb2 as pb            # noqa: E402

PFX = sys.argv[1] if len(sys.argv) > 1 else r"C:\dev\aoe2\aoe2grpc\run1"
...
def main():
    ...
    f0 = rows[0]
    fc = next((r["game_s"] for r in rows
               if (r["side1"]["count"], r["side2"]["count"])
               != (f0["side1"]["count"], f0["side2"]["count"])), None)
    with open(PFX + ".fight.json", "w") as o:
        json.dump({"segments": len(segments), "start_counts": best.start_counts,
                   "rows": len(rows), "end_game_s": end_s,
                   "first_change_game_s": fc}, o)
