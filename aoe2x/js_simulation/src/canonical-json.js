// The combat engine is shared by Node batch jobs and a browser Web Worker.
// Keep Node's native implementation for throughput and use the synchronous,
// byte-identical fallback below in browsers.
let createNodeHash = null;
if (typeof process !== "undefined" && process?.versions?.node) {
  ({ createHash: createNodeHash } = await import("node:crypto"));
}
...
function browserSha256(text) {
  ...
  for (let offset = 0; offset < bytes.length; offset += 64) {
    ...
    for (let index = 0; index < 64; index += 1) {
      ...
      const temp1 = (h + s1 + choose + SHA256_ROUND[index] + words[index]) >>> 0;
...
export function hashCanonicalJson(value) {
  ...
  return createNodeHash
    ? createNodeHash("sha256").update(serialized).digest("hex")
    : browserSha256(serialized);
}
