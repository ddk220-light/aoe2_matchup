...
def export_unit_mechanics(
    ...
    *,
    ...
    reference = dict(reference_row) if reference_row is not None else _read_reference_row(reference_db, unit_slug, civ)
    ...
    immediate_self_destruct = (
        frame_delay == 0
        and int(unit.type_50.blast_attack_level) == 66
        and int(unit.type_50.attack_graphic) == int(unit.dying_graphic)
    )
    if immediate_self_destruct:
        # This is the death graphic, not a melee swing. All 19 observed
        # Flaming Camels die on the next 14-18 ms capture sample after this
        # graphic starts; waiting for its .25-second midpoint is incorrect.
        attack_delay_seconds = 0.0
        attack_delay_source = (
            "zero-frame self-destruct with attack_graphic == dying_graphic: "
            "immediate death boundary, confirmed by archived 14-18 ms samples"
        )
    elif immediate_blast_projectile:
        attack_delay_seconds = 0.0
        attack_delay_source = (
    ...
    if unit.creatable.charge_type in (4, 5) and unit.creatable.max_charge > 0:
        prefix = 'melee_block' if unit.creatable.charge_type == 5 else 'projectile_dodge'
        runtime_effects[prefix + '_max'] = float(unit.creatable.max_charge)
        runtime_effects[prefix + '_recharge_per_second'] = float(unit.creatable.recharge_rate)
        runtime_effects.pop('block_first_melee', None)
        fields['effects.' + prefix] = 'unit.creatable charge_type, max_charge, recharge_rate'
    aura_tasks = [t for t in unit.bird.tasks if t.action_type == 155
                  and t.search_wait_time == 9 and t.work_value_1 > 0 and t.work_value_2 > 0]
    if aura_tasks:
        task = aura_tasks[0]
        runtime_effects.update(attack_aura_radius=float(task.work_range),
            attack_aura_group_size=float(task.work_value_2 / task.work_value_1),
            attack_aura_max=float(task.work_value_1),
            attack_aura_masters=sorted({t.unit_id for t in aura_tasks if t.unit_id >= 0}))
        fields['effects.attack_aura'] = 'unit.bird.tasks[action_type=155, attribute=9]; work_value_2/work_value_1 units per attack point'
    if unit.type_50.blast_attack_level == 66:
        runtime_effects['suicide_attack'] = True
        fields['effects.suicide_attack'] = 'unit.type_50.blast_attack_level == 66 (self-destruct blast)'
    if not attack_classes:
        runtime_effects['non_attacking'] = True
        fields['effects.non_attacking'] = 'DAT contains no damage attack classes; conversion requires an explicit command'
    if form_stats:
        runtime_effects = dict(runtime_effects or {})
    ...
    return {
        ...
        "frame_delay": int(unit.type_50.frame_delay),
        "derived": {"damage_vs_self": _damage_against_self(attack_classes, armor_classes) if attack_classes else 0},
        "provenance": {
            "reference_db_sha256": _sha256(reference_db),
            "dat_sha256": _sha256(dat_path),
            "reference_selector": (
                reference.get('source_selector') or
                f"ref_units WHERE unit_slug='{unit_slug}' AND civ_name='{civ}' AND age='Imperial'"
            ),
            "dat_selector": (
