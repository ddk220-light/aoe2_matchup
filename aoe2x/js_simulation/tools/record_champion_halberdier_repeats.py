"""Record fresh Champion-vs-Halberdier game trials from the current melee golden.
...
STEM = "spanish_champion_vs_halberdier"
EXPECTED_COUNTS = (23, 27)
...
def validate_generated_scenario(path: Path) -> dict:
    ...
    halberdier = unit_const("halberdier")
    source_masters = {
    ...
    actual_positions = {
        ...
        3: army_positions(generated, 3, halberdier),
    }
    ...
    return {
        "sha256": sha256(path),
        "counts": {"player2_champions": 23, "player3_halberdiers": 27},
...
def validate_capture(run_dir: Path) -> dict:
    ...
    elimination = next(row for row in rows if row["side2"]["count"] == 0)
    return {
        "game_version": sidecar.get("game_version"),
        "start_counts": list(start),
        "winner": "champion",
        "survivors": end[0],
        "winner_hp": final["side1"]["hp"],
...
def main() -> None:
    ...
    batch = {
        ...
        "matchup": {
            "player2": {"civ": "Spanish", "unit": "Champion", "count": 23},
            "player3": {"civ": "Spanish", "unit": "Halberdier", "count": 27},
    ...
    for repeat in range(1, args.repeats + 1):
        ...
        raw_path = run_matchup(
            ...
            compose=False,
            out_mov=str(Path(tempfile.gettempdir()) / f"aoe2_champion_halberdier_{repeat:03d}.mov"),
            final=str(Path(tempfile.gettempdir()) / f"aoe2_champion_halberdier_{repeat:03d}.mp4"),
            dismiss_after=True,
            logfile=str(log_path),
            template=GOLDEN,
        )
        generated = RUN_DIR / "champion_vs_halberdier.aoe2scenario"
