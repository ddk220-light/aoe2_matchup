"""Record fresh Spanish Paladin-vs-Burmese Elite Battle Elephant trials.

This is a thin batch driver around the established video/gRPC automation. It
pins the verified project-local current melee golden and validates every
generated first-N roster and raw gRPC sidecar before advancing.
"""
...
DEFAULT_OUTPUT = (
    ...
    / "calibration"
    / "live_observations"
    / "spanish_paladin_vs_burmese_elephant_5x_2026-08-28"
)
STEM = "spanish_paladin_vs_burmese_elephant"
EXPECTED_COUNTS = (27, 21)
...
def validate_generated_scenario(path: Path) -> dict:
    ...
    elephant = unit_const("elite_elephant")
    source_masters = {
    ...
    actual_positions = {
        2: army_positions(generated, 2, paladin),
        3: army_positions(generated, 3, elephant),
    }
    ...
    return {
        "sha256": sha256(path),
        "counts": {"player2_paladins": 27, "player3_elite_battle_elephants": 21},
...
def validate_capture(run_dir: Path) -> dict:
    ...
    winner_side = "paladin" if end[0] > 0 else "elite_elephant"
...
def main() -> None:
    ...
    batch = {
        ...
        "matchup": {
            "player2": {"civ": "Spanish", "unit": "Paladin", "count": 27},
            "player3": {
                "civ": "Burmese", "unit": "Elite Battle Elephant", "count": 21
            },
            "purchase_rule": (
                "unweighted resources; cheaper side capped at 27 and pricier side floored"
            ),
            "resource_totals": {"player2": 3645, "player3": 3570},
        },
        "runs": [],
    }
    ...
    for repeat in range(1, args.repeats + 1):
        ...
        raw_path = run_matchup(
            ...
            compose=False,
            out_mov=str(Path(tempfile.gettempdir()) / f"aoe2_paladin_elephant_{repeat:03d}.mov"),
            final=str(Path(tempfile.gettempdir()) / f"aoe2_paladin_elephant_{repeat:03d}.mp4"),
            dismiss_after=True,
            logfile=str(log_path),
            template=GOLDEN,
        )
        generated = RUN_DIR / "paladin_vs_elite_elephant.aoe2scenario"
