...
def build_parser() -> argparse.ArgumentParser:
    ...
    live.add_argument("--mode", choices=("statistics", "recorder"), default="statistics")

    record = sub.add_parser("record", help="capture raw video and gRPC in an expanded folder")
    _matchup_arguments(record)
    record.add_argument("--repeats", type=int, default=1)
    record.add_argument("--retries", type=int, default=1)
    record.set_defaults(mode="recorder", retention=None)
    ...
    full.add_argument("--mode", choices=("statistics", "recorder"), default="statistics")
    ...
    batch.add_argument("--phase", choices=("simulate", "live", "full", "recorder"), default="simulate")
...
def _run(args: argparse.Namespace, config: LabConfig) -> int:
    if args.command in ("live", "record", "run"):
        args.retention = recorder_retention(args.mode, args.retention)
    elif args.command == "batch" and args.phase == "recorder":
        args.retention = recorder_retention("recorder", args.retention)
    ...
    if args.command in ("simulate", "sim"):
        ...
        return 0
    if args.command in ("live", "record"):
        job, plan = _job(config, _request_from_args(args))
        try:
            summary = run_live(
                config,
    ...
    if args.command == "batch":
        ...
        live_summaries = {}
        if args.phase in ("live", "full", "recorder"):
            ...
            for job in jobs:
                try:
                    live_summaries[job.job_id] = run_live(
                        ...
                        retries=args.retries,
                        retention=args.retention,
                        mode="recorder" if args.phase == "recorder" else "statistics",
                    )
