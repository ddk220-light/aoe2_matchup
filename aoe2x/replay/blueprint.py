#!/usr/bin/env python3
"""Role: replay — THE replay viewer, as a mountable Flask Blueprint.

Defines `replay_bp`, the single canonical AoE2 replay viewer backend
(replay upload/download + parsing via mgz, unit-type reconstruction via
unit_classifier, map/playback payloads for the canvas SPA in public/, and
the WebM highlight-clip export in clip_export.py). Routes carry NO mount
prefix — each app picks one at registration:

    # standalone viewer (apps/viewer/server.py) — URLs match the old
    # aoe2record visualizer exactly (/, /api/*):
    app.register_blueprint(replay_bp)

    # embedded in the matchup website (webapp/app.py) — URLs match the old
    # analyzer fork exactly (/replay/, /replay/api/*):
    app.register_blueprint(replay_bp, url_prefix="/replay")
    app.config["REPLAY_VIEW_URL"] = "/replay"   # shell page for share links

The SPA fetches its API with RELATIVE urls, so one public/app.js serves
both mounts. This module replaced the historical fork pair
(aoe2record visualizer/server.py <-> analyzer webapp/replay_core.py) on
2026-06-11; docs/ANALYZER_SYNC.md is retired.
"""
...
# The SPA bundle (index.html, app.js, renderer.js, playback.js, sprites...)
PUBLIC_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "public")

# Prefix-free blueprint; the mounting app chooses the URL prefix.
...
@replay_bp.route("/<path:path>")
def static_files(path):
    """SPA assets + generated clips. Werkzeug routing prefers the specific
    /api/* rules over this catch-all."""
    return send_from_directory(PUBLIC_DIR, path)


@replay_bp.route("/api/default", methods=["GET"])
def get_default_replay():
    """Return the default replay_data.json if it exists (dev convenience)."""
    try:
...
def load_players_from_csv():
    ...
    try:
        ...
        log.warning(f"Players CSV not found: {PLAYERS_CSV_PATH}")
    return players
...
def process_replay(replay_file):
    ...
    try:
        from aoe2x.replay import unit_classifier as _uc
        unit_type_map, _remap = _uc.build_type_map(match)
        _canon = lambda o: _remap.get(o, o)  # noqa: E731  collapse shifted dups
    except Exception as e:
        log.warning(f"v2 classification failed, falling back to legacy: {e}")
        try:
            unit_type_map = _classify_units(match)
        except Exception as e2:
            log.warning(f"legacy classification also failed: {e2}")
            unit_type_map = {}
    ...
    try:
        ...
        log.warning(f"building analysis failed: {e}")
    ...
    try:
        ...
        log.warning(f"terrain/object extraction failed: {e}")
        data["terrain"] = None
        data["map_objects"] = []
        data["animals"] = []
...
@replay_bp.route("/api/upload", methods=["POST"])
def upload_replay():
...
@replay_bp.route("/api/matches", methods=["GET"])
def get_matches():
...
@replay_bp.route("/api/matches/<player_name>", methods=["GET"])
...
@replay_bp.route("/api/players", methods=["GET"])
def search_players():
...
@replay_bp.route("/api/player/<int:profile_id>/matches", methods=["GET"])
...
def _fetch_replay_to_cache(match_id, profile_id):
    ...
    if os.path.exists(cache_path) and os.path.getsize(cache_path) > 0:
        log.info(f"Replay cache hit: {cache_path}")
        return cache_path

    download_url = f"{REPLAY_DOWNLOAD_URL}/?gameId={match_id}&profileId={profile_id}"
    log.info(f"Downloading replay from: {download_url}")
...
@replay_bp.route("/api/load-match", methods=["POST"])
def load_match():
...
CLIP_DIR = os.path.join(PUBLIC_DIR, "clips")


@replay_bp.route("/api/clip", methods=["GET"])
def make_clip():
    ...
    if not (os.path.exists(out_path) and os.path.getsize(out_path) > 0):
        try:
            from aoe2x.replay import clip_export
            with open(cache_path, "rb") as f:
    ...
    # Mount-aware: url_for resolves against this blueprint's registered prefix
    # (/clips/x.webm standalone, /replay/clips/x.webm embedded).
    clip_path = url_for(".static_files", path=f"clips/{out_name}")
    # The ?match=&profile= deep-link params are read by app.js init(). An
    # embedding app may point share links at its shell page (e.g. the
    # website's /replay tab, which keeps the site nav visible) via
    # app.config["REPLAY_VIEW_URL"]; default is the SPA index itself.
    view_base = current_app.config.get("REPLAY_VIEW_URL") or url_for(".index").rstrip("/")
    return jsonify({
        "clip_url": f"{base}{clip_path}",
        "view_url": f"{base}{view_base}?match={match_id}&profile={profile_id}",
    })
