...
| Symptom | Cause | Fix |
|---|---|---|
| A clip shows the WRONG matchup / wrong armies; gRPC gate rejects it; counts decode as the template default (30v30) | The Load-Scenario nav clicked a blind fixed ROW coord, but extra files in the scenario folder (default1/default3/The Siege/…) pushed the staged "Matchup Run" off the top row — so the game test-played the template instead | Fixed: the row is now selected BY NAME (OCR-find "Matchup Run"), fixed coord is the fallback. Lesson: the gRPC sanity gate is the tripwire that caught this — trust it, and when counts decode as the template's own army, suspect scenario loading, not the decoder |
