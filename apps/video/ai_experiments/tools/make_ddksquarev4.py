"""Generate ddkSquareV4.per = ddkSquareV3 (5-tiles-from-edge CCW loop) but with
MINIMAL DEBUG OUTPUT: the chat prints ONLY the target x,y it is moving toward, and
only when that target CHANGES.  All other chatter is silenced -- both my own extra
lines (s=/off=/banner) and the base AI's frequent KITE/KITE2/VOLLEY/MOVE-P/MOVE-M
lines that fire every transition/move.

up-chat-data-to-player prints one value per call, so each new target shows as an
"x=<X>" line followed by a "y=<Y>" line.  (Base one-time chats -- TAGGED, WFIRE SET,
ENEMY=P3, D1..D7 -- still fire ONCE at battle start, then the stream is only x/y.)

Geometry identical to V3 (INSET=5 -> loop [500,1100], sides 6 tiles, arc 0..2400, CCW).
GRAFT: rules 0-77 untouched except 44/45/46 (drop transition chats), 56/57 (target ->
vecSquare + drop move chats), 65 (drop no-targets chat).
Validation: validate_variant.py ddkModelAI.per ddkSquareV4.per 44,45,46,56,57,65 17
"""
import io
...
DST = r"apps\video\ai_experiments\ddkSquareV4.per"
...
rep("; ddkModelAI = CoreF + per-unit kiting character + broad ranged recognition +",
    f"; ddkSquareV4 = ddkSquareV3 ({INSET} tiles from edge, CCW loop [{LO},{HI}]) with\n"
    ";   MINIMAL LOGGING: chat prints ONLY the target x,y (on change).  All frequent\n"
    ";   base chats (KITE/KITE2/VOLLEY/MOVE-P/MOVE-M/no-targets) are stripped.\n"
    ";\n"
...
# ---------- APPENDIX (17 rules) ----------
ap = []
ap.append(f""";=== ddkSquareV4 CCW SQUARE-TRACK APPENDIX ({INSET} from edge; loop [{LO},{HI}],
;    arc 0..{PERIM}).  Minimal logging: only target x,y on change.  goals 202-219. ===
...
ap.append(""";--- LOG: only when the target moves, print x then y (nothing else) ---
(defrule
\t(up-compare-goal gTagged c:== 1)
...
\t(up-chat-data-to-player 1 "x=%d" g: vecSquare_x)
\t(up-chat-data-to-player 1 "y=%d" g: vecSquare_y)
)""")
...
print(f"wrote {DST}: {len(text)} chars, {n_rules} defrules (expect 139), "
      f"loop=[{LO},{HI}] perim={PERIM}, NL={NL!r}")
