"""Assemble all archived Elite Blackwood Archer overlays with the reviewed-format intro."""
...
OUT=LAB/'compilations/blackwood-archer-unique-units/final';OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ff=find_ffmpeg();state=json.loads((LAB/'campaigns/blackwood-archer-all-unique-overlays/status.json').read_text());clips=[];results=[]
 intro=LAB/'compilations/blackwood-archer-unique-units/intro-v1/blackwood-archer-intro.mp4';normalized=OUT/'intro-1440p60.mp4'
 with (OUT/'render.log').open('w') as log:
  ...
  meta=';FFMETADATA1\ntitle=Elite Blackwood Archer vs 73 Unique Units\n'
  ...
  video=OUT/'blackwood-archer-complete-with-intro.mp4'
