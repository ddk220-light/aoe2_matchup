"""Assemble all archived Flaming Camel overlays with the reviewed-format intro."""
...
OUT=LAB/'compilations/flaming-camel-unique-units/final';OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ff=find_ffmpeg();state=json.loads((LAB/'campaigns/flaming-camel-all-unique-overlays/status.json').read_text());clips=[];results=[]
 intro=LAB/'compilations/flaming-camel-unique-units/intro-v1/flaming-camel-intro.mp4';normalized=OUT/'intro-1440p60.mp4'
 with (OUT/'render.log').open('w') as log:
  ...
  meta=';FFMETADATA1\ntitle=Flaming Camel vs 73 Unique Units\n'
  ...
  video=OUT/'flaming-camel-complete-with-intro.mp4'
  ...
  desc="Flaming Camel vs 73 Unique Units | Age of Empires II DE\n\nRecorded in-game battles with live unit HP overlays. Equal resources, maximum 27 units per main army. All battles use the three-player arena with no extra frontline buffer, including against ranged opponents. Whole-unit rounding applies. \n\nTry your own matchup: https://aoe2matchup.com/?civ1=Tatars&age1=Imperial\n\nWIN/LOSS is from Flaming Camel's perspective; HP is the winning army's remaining HP. Missionary totals include converted units.\n\n00:00 Introduction\n"
  desc+='\n'.join(f"{r['timestamp']} {r['unit']} | {r['result']} | {round(r['winnerHp'])} HP" for r in results)
  desc+='\n\n#AoE2 #AoE2DE #FlamingCamel #Tatars #RTS #BattleSimulation #UnitCounters #StrategyGaming\n'
