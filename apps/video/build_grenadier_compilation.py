"""Assemble all archived Grenadier overlays with the reviewed-format intro."""
...
OUT=LAB/'compilations/grenadier-unique-units/final';OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ff=find_ffmpeg();state=json.loads((LAB/'campaigns/grenadier-all-unique-overlays/status.json').read_text());clips=[];results=[]
 intro=LAB/'compilations/grenadier-unique-units/intro-v1/grenadier-intro.mp4';normalized=OUT/'intro-1440p60.mp4'
 with (OUT/'render.log').open('w') as log:
  ...
  meta=';FFMETADATA1\ntitle=Grenadier vs 73 Unique Units\n'
  ...
  video=OUT/'grenadier-complete-with-intro.mp4'
  ...
  desc="Grenadier vs 73 Unique Units | Age of Empires II DE\n\nRecorded in-game battles with live unit HP overlays. Equal resources, maximum 27 units per main army. Ranged units get a small front line of Hussars when fighting melee units. Screen units are excluded from the main-army HP totals. Whole-unit rounding applies. \n\nTry your own matchup: https://aoe2matchup.com/?civ1=Jurchens&unit1=grenadier_jurchens&age1=Imperial\n\nWIN/LOSS is from Grenadier's perspective; HP is the winning army's remaining HP. Missionary totals include converted units.\n\n00:00 Introduction\n"
  desc+='\n'.join(f"{r['timestamp']} {r['unit']} | {r['result']} | {round(r['winnerHp'])} HP" for r in results)
  desc+='\n\n#AoE2 #AoE2DE #Grenadier #Jurchens #RTS #BattleSimulation #UnitCounters #StrategyGaming\n'
