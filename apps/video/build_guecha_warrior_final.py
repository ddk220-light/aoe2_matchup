"""Assemble all archived Elite Guecha Warrior overlays with the reviewed-format intro."""
...
OUT=LAB/'compilations/guecha-warrior-unique-units/final-cost-v2';OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ff=find_ffmpeg();state=json.loads((LAB/'campaigns/guecha-warrior-final-overlays/status.json').read_text());clips=[];results=[]
 intro=LAB/'compilations/guecha-warrior-unique-units/intro-v1/guecha-warrior-intro.mp4';normalized=OUT/'intro-1440p60.mp4'
 with (OUT/'render.log').open('w') as log:
  ...
  meta=';FFMETADATA1\ntitle=Elite Guecha Warrior vs 73 Unique Units\n'
  ...
  video=OUT/'guecha-warrior-complete-corrected-costs.mp4'
  ...
  (OUT/'manifest.json').write_text(json.dumps({'output':str(video),'matchups':73,'chapters':74,'expectedSeconds':cursor,'media':info,'fullDecode':'passed','reviewStatus':'Automated media checks passed; template reviewed','results':results},indent=2))
  desc="Elite Guecha Warrior vs 73 Unique Units | Age of Empires II DE\n\nRecorded in-game battles with live unit HP overlays. Equal resources using civilization-specific Imperial costs per individual unit, maximum 27 units per main army. Ranged units get a small front line of hussars when fighting melee units. Whole-unit rounding applies.\n\nTry your own matchup: https://aoe2matchup.com/?civ1=Muisca&unit1=elite_guecha_warrior_muisca&age1=Imperial\n\nWIN/LOSS is from Elite Guecha Warrior's perspective; HP is the winning army's remaining HP.\n\n00:00 Introduction\n"
  desc+='\n'.join(f"{r['timestamp']} {r['unit']} | {r['result']} | {round(r['winnerHp'])} HP" for r in results)
  desc+='\n\n#AoE2 #AoE2DE #Guechawarrior #Muisca #RTS #BattleSimulation #UnitCounters #StrategyGaming\n'
