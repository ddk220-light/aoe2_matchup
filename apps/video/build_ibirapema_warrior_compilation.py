"""Assemble all archived Elite Ibirapema Warrior overlays with the reviewed-format intro."""
...
OUT=LAB/'compilations/ibirapema-warrior-unique-units/final';OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ff=find_ffmpeg();state=json.loads((LAB/'campaigns/ibirapema-warrior-all-unique-overlays/status.json').read_text());clips=[];results=[]
 intro=LAB/'compilations/ibirapema-warrior-unique-units/intro-v1/ibirapema-warrior-intro.mp4';normalized=OUT/'intro-1440p60.mp4'
 with (OUT/'render.log').open('w') as log:
  ...
  meta=';FFMETADATA1\ntitle=Elite Ibirapema Warrior vs 73 Unique Units\n'
  ...
  video=OUT/'ibirapema-warrior-complete-with-intro.mp4'
  ...
  desc="Elite Ibirapema Warrior vs 73 Unique Units | Age of Empires II DE\n\nRecorded in-game battles with live unit HP overlays. Equal resources, maximum 27 units per main army. Ranged units get a small front line of hussars when fighting melee units. Whole-unit rounding applies.\n\nTry your own matchup: https://aoe2matchup.com/?civ1=Tupi&unit1=elite_ibirapema_warrior_tupi&age1=Imperial\n\nWIN/LOSS is from Elite Ibirapema Warrior's perspective; HP is the winning army's remaining HP.\n\n00:00 Introduction\n"
  desc+='\n'.join(f"{r['timestamp']} {r['unit']} | {r['result']} | {round(r['winnerHp'])} HP" for r in results)
  desc+='\n\n#AoE2 #AoE2DE #IbirapemaWarrior #Tupi #RTS #BattleSimulation #UnitCounters #StrategyGaming\n'
