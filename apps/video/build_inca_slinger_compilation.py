"""Assemble all archived Inca Slinger overlays with the reviewed-format intro."""
...
OUT=LAB/'compilations/inca-slinger-unique-units/final';OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ff=find_ffmpeg();state=json.loads((LAB/'campaigns/inca-slinger-all-unique-overlays/status.json').read_text());clips=[];results=[]
 ...
 with (OUT/'render.log').open('w') as log:
  ...
  meta=';FFMETADATA1\ntitle=Inca Slinger vs 74 Unique Units\n'
  ...
  video=OUT/'inca-slinger-complete-with-intro.mp4'
  ...
  info=probe(video);assert abs(float(info['format']['duration'])-cursor)<2;assert len(info['chapters'])==75
  call(['-threads','2','-i',str(video),'-f','null','-'])
  (OUT/'manifest.json').write_text(json.dumps({'output':str(video),'matchups':74,'chapters':75,'expectedSeconds':cursor,'media':info,'fullDecode':'passed','reviewStatus':'Locally validated; authorized for private YouTube upload','results':results},indent=2))
  desc="Inca Slinger vs 74 Unique Units | Age of Empires II DE\n\nRecorded in-game battles with live unit HP overlays. Equal resources, maximum 27 units per main army. Ranged units get a small front line of Hussars when fighting melee units. Screen units are excluded from the main-army HP totals. Whole-unit rounding applies. \n\nTry your own matchup: https://aoe2matchup.com/?civ1=Incas&unit1=imp_slinger&age1=Imperial\n\nWIN/LOSS is from Inca Slinger's perspective; HP is the winning army's remaining HP. Missionary totals include converted units.\n\n00:00 Introduction\n"
  desc+='\n'.join(f"{r['timestamp']} {r['unit']} | {r['result']} | {round(r['winnerHp'])} HP" for r in results)
  desc+='\n\n#AoE2 #AoE2DE #IncaSlinger #Incas #RTS #BattleSimulation #UnitCounters #StrategyGaming\n'
