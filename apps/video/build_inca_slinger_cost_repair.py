...
OUT=LAB/'compilations/inca-slinger-unique-units/final-cost-v2'
...
def main():
 ...
 repair=next(x for x in read(ROOT/'data/local/production-audit/repair-plan.json')['compilations'] if x['episode']=='inca-slinger-unique-units')
 old=read(LAB/'compilations/inca-slinger-unique-units/final/manifest.json');oldrows={r['jobId']:r for r in old['results']}
 overlays=read(LAB/'campaigns/inca-slinger-cost-v2-overlays/status.json');assert overlays['state']=='COMPLETE' and overlays['failed']==0
 with (OUT/'render.log').open('a') as log:
  ...
  intro=LAB/'compilations/inca-slinger-unique-units/intro-v1/inca-slinger-intro.mp4';normalized=OUT/'intro-1440p60.mp4'
  ...
  metadata=';FFMETADATA1\ntitle=Inca Slinger vs 74 Unique Units - Corrected Costs\n'
  ...
  video=OUT/'inca-slinger-complete-corrected-costs.mp4'
