"""Assemble all archived Elite Fire Lancer overlays with the reviewed-format intro."""
...
OUT=LAB/'compilations/korean-fire-lancer-unique-units/final';OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ff=find_ffmpeg();state=json.loads((LAB/'campaigns/korean-fire-lancer-all-unique-overlays/status.json').read_text());clips=[];results=[]
 ...
 with (OUT/'render.log').open('w') as log:
  ...
  meta=';FFMETADATA1\ntitle=Elite Fire Lancer vs 74 Unique Units\n'
  ...
  video=OUT/'korean-fire-lancer-complete-with-intro.mp4'
