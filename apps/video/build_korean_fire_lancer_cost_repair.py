...
OUT=LAB/'compilations/korean-fire-lancer-unique-units/final-cost-v2'
...
def main():
 ...
 repair=next(x for x in read(ROOT/'data/local/production-audit/repair-plan.json')['compilations'] if x['episode']=='korean-fire-lancer-unique-units')
 old=read(LAB/'compilations/korean-fire-lancer-unique-units/final/manifest.json');oldrows={r['jobId']:r for r in old['results']}
 overlays=read(LAB/'campaigns/korean-fire-lancer-cost-v2-overlays/status.json');assert overlays['state']=='COMPLETE' and overlays['failed']==0
 with (OUT/'render.log').open('a') as log:
  ...
  intro=LAB/'compilations/korean-fire-lancer-unique-units/intro-v1/korean-fire-lancer-intro.mp4';normalized=OUT/'intro-1440p60.mp4'
  ...
  metadata=';FFMETADATA1\ntitle=Elite Fire Lancer vs 74 Unique Units - Corrected Costs\n'
  ...
  video=OUT/'korean-fire-lancer-complete-corrected-costs.mp4'
  ...
  info=probe(video);assert len(info['chapters'])==75 and abs(float(info['format']['duration'])-cursor)<2
  call(['-threads','2','-i',video,'-f','null','-'])
  write_json(OUT/'manifest.json',{'output':str(video),'matchups':74,'chapters':75,'media':info,'fullDecode':'passed','reviewStatus':'NEEDS_VISUAL_QA','results':results,'costBasis':'fully_upgraded_imperial_v1'})
  desc='Elite Fire Lancer vs 74 Unique Units | Age of Empires II DE\n\nEqual resources using civilization-specific Imperial costs per individual unit; maximum 27 units per main army. Ranged units get a small front line of hussars against melee units. Whole-unit rounding applies.\n\nTry your own matchup: https://aoe2matchup.com/?civ1=Koreans&unit1=elite_fire_lancer&age1=Imperial\n\nWIN/LOSS is from the Elite Fire Lancer perspective; HP is the winning army remaining HP.\n\n00:00 Introduction\n'
  desc+='\n'.join(f"{r['timestamp']} {r['unit']} | {r['result']} | {round(r['winnerHp'])} HP" for r in results)
  desc+='\n\n#AoE2 #AoE2DE #Koreans #FireLancer #RTS #BattleSimulation #UnitCounters\n';assert len(desc)<=5000
