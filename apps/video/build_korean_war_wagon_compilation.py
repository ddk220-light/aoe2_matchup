"""Assemble all archived Elite War Wagon overlays with the reviewed-format intro."""
...
OUT=LAB/'compilations/korean-war-wagon-unique-units/final';OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ff=find_ffmpeg();state=json.loads((LAB/'campaigns/korean-war-wagon-all-unique-overlays/status.json').read_text());clips=[];results=[]
 ...
 with (OUT/'render.log').open('w') as log:
  ...
  meta=';FFMETADATA1\ntitle=Elite War Wagon vs 73 Unique Units\n'
  ...
  video=OUT/'korean-war-wagon-complete-with-intro.mp4'
