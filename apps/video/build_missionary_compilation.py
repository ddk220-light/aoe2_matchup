"""Assemble all archived Missionary overlays with the reviewed-format intro."""
...
OUT=LAB/'compilations/missionary-unique-units/final';OUT.mkdir(parents=True,exist_ok=True)
...
def reusable_trim(path, source, duration):
 if not path.exists() or path.stat().st_size == 0 or path.stat().st_mtime < source.stat().st_mtime:return False
 try:
  info=probe(path);video=next(s for s in info['streams'] if s['codec_type']=='video')
  return (abs(float(info['format']['duration'])-duration)<.1 and video['codec_name']=='h264'
          and video['width']==2560 and video['height']==1440
          and video['pix_fmt']=='yuv420p' and video['r_frame_rate']=='60/1'
          and any(s['codec_type']=='audio' for s in info['streams']))
 except (subprocess.CalledProcessError,KeyError,ValueError,StopIteration):return False
def main():
 ff=find_ffmpeg();state=json.loads((LAB/'campaigns/missionary-all-unique-overlays/status.json').read_text());clips=[];results=[]
 intro=LAB/'compilations/missionary-unique-units/intro-v1/missionary-intro.mp4';normalized=OUT/'intro-1440p60.mp4'
 with (OUT/'render.log').open('w') as log:
  ...
  for j in state['jobs']:
   ...
   if float(probe(source)['format']['duration']) > end['videoSeconds'] + .5:
    ...
    if not reusable_trim(trimmed,source,duration):
     partial=trimmed.with_suffix('.partial.mp4')
     call(['-threads','2','-i',str(source),'-t',str(duration),'-map','0:v:0','-map','0:a:0','-c:v','h264_nvenc','-preset','p4','-rc','vbr','-cq','18','-b:v','0','-pix_fmt','yuv420p','-c:a','aac','-b:a','192k',str(partial)])
     assert reusable_trim(partial,source,duration),partial
     partial.replace(trimmed)
    source=trimmed
  ...
  meta=';FFMETADATA1\ntitle=Missionary vs 73 Unique Units\n'
  ...
  video=OUT/'missionary-complete-with-intro.mp4'
  ...
  desc="Missionary vs 73 Unique Units | Age of Empires II DE\n\nRecorded in-game battles with live unit HP overlays. Equal resources, maximum 27 units per main army. Ranged units get a small front line of hussars when fighting melee units. Whole-unit rounding applies.\n\nTry your own matchup: https://aoe2matchup.com/?civ1=Spanish&age1=Imperial\n\nWIN/LOSS is from Missionary's perspective; HP is the winning army's remaining HP, including converted units.\n\n00:00 Introduction\n"
  desc+='\n'.join(f"{r['timestamp']} {r['unit']} | {r['result']} | {round(r['winnerHp'])} HP" for r in results)
  desc+='\n\n#AoE2 #AoE2DE #Missionary #Spanish #RTS #BattleSimulation #UnitCounters #StrategyGaming\n'
