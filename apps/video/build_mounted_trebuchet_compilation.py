"""Assemble all archived Mounted Trebuchet overlays with the reviewed-format intro."""
...
OUT=LAB/'compilations/mounted-trebuchet-unique-units/final';OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ff=find_ffmpeg();state=json.loads((LAB/'campaigns/mounted-trebuchet-all-unique-overlays/status.json').read_text());clips=[];results=[]
 intro=LAB/'compilations/mounted-trebuchet-unique-units/intro-v1/mounted-trebuchet-intro.mp4';normalized=OUT/'intro-1440p60.mp4'
 with (OUT/'render.log').open('w') as log:
  ...
  for j in state['jobs']:
   ...
   if j['unit']=='Missionary':
    ...
    trimmed=OUT/'missionary-battle-end.mp4'
  ...
  meta=';FFMETADATA1\ntitle=Mounted Trebuchet vs 33 Ranged Unique Units\n'
  ...
  video=OUT/'mounted-trebuchet-complete-with-intro.mp4'
  ...
  info=probe(video);assert abs(float(info['format']['duration'])-cursor)<2;assert len(info['chapters'])==34
  call(['-threads','2','-i',str(video),'-f','null','-'])
  (OUT/'manifest.json').write_text(json.dumps({'output':str(video),'matchups':33,'chapters':34,'expectedSeconds':cursor,'media':info,'fullDecode':'passed','reviewStatus':'Locally validated; authorized for private YouTube upload','results':results},indent=2))
  desc="Mounted Trebuchet vs 33 Ranged Unique Units | Age of Empires II DE\n\nRecorded in-game battles with live unit HP overlays. Equal resources, maximum 27 units per main army. Only ranged opponents are included. All battles use the three-player arena without an extra frontline buffer. Whole-unit rounding applies. \n\nTry your own matchup: https://aoe2matchup.com/?civ1=Khitans&unit1=mounted_trebuchet_khitans&age1=Imperial\n\nWIN/LOSS is from Mounted Trebuchet's perspective; HP is the winning army's remaining HP. Missionary totals include converted units.\n\n00:00 Introduction\n"
  desc+='\n'.join(f"{r['timestamp']} {r['unit']} | {r['result']} | {round(r['winnerHp'])} HP" for r in results)
  desc+='\n\n#AoE2 #AoE2DE #MountedTrebuchet #Khitans #RTS #BattleSimulation #UnitCounters #StrategyGaming\n'
