"""Assemble all archived Elite Temple Guard overlays with the reviewed-format intro."""
...
OUT=LAB/'compilations/temple-guard-unique-units/final-cost-v2';OUT.mkdir(parents=True,exist_ok=True)
...
def main():
 ff=find_ffmpeg();state=json.loads((LAB/'campaigns/temple-guard-final-overlays/status.json').read_text());clips=[];results=[]
 intro=LAB/'compilations/temple-guard-unique-units/intro-v1/temple-guard-intro.mp4';normalized=OUT/'intro-1440p60.mp4'
 with (OUT/'render.log').open('w') as log:
  ...
  meta=';FFMETADATA1\ntitle=Elite Temple Guard vs 73 Unique Units\n'
  ...
  video=OUT/'temple-guard-complete-corrected-costs.mp4'
