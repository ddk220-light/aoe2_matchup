"""Choose ten Elite Champi Warrior Shorts from recorded outcomes and persisted unit costs."""
import argparse,json,sqlite3
from pathlib import Path
from overlay.battle_end import terminal_row
LAB=Path('aoe2x/js_simulation/calibration/lab').resolve()
OUT=LAB/'shorts/champi-warrior-selected-10'

def main():
    p=argparse.ArgumentParser();p.add_argument('--early',action='store_true');a=p.parse_args()
    state=json.loads((LAB/'campaigns/champi-warrior-final-overlays/status.json').read_text())
    ...
    for j in state['jobs']:
        ...
        inventory.append(dict(subject='champi-warrior',jobId=j['jobId'],unit=s['label'],civ=s['civ'],cost=sum(json.loads(Path('data/recording-costs.json').read_text())['units'][s['civ']+'|'+s['slug']]['effectiveCost'].values()),category=stats.get('unit_class_name','Special'),ranged=s['ranged'],winner=winner,winnerHp=sum(u['hp'] for u in last['sides'][str(winner)]) if winner else 0,run=str(run)))
    ...
    for name,reason in [('Missionary','Conversion pressure against the featured unit'),('Flaming Camel','Explosive charge against the featured unit'),('Houfnice','Long-range siege matchup'),('Elite Tiger Cavalry','Kill-based growth against the featured unit'),('Elite War Elephant','High-HP cavalry matchup'),('Elite Teutonic Knight','Heavily armored infantry closing the gap')]:
    ...
    assert all(x['unit']!='Elite Champi Warrior' for x in selected)
