"""Prepare the approved Elite Monaspa capture roster with audited unit costs."""
import json
...
def main():
    ...
    subject = next(x for x in roster if x['slug'] == 'elite_monaspa_georgians')
    opponents = sorted((x for x in roster if x['slug'] != subject['slug']), key=lambda x: (x['civ'].casefold(), x['label'].casefold()))
    rows = [dict(id=f'elite_monaspa_unique_{i:02}_{safe_slug(x["civ"])}_{safe_slug(x["slug"])}',
    ...
    manifest = ROOT / 'aoe2lab.recorder.elite-monaspa-all-unique.json'
    ...
    for request in requests:
        ...
        plans.append(plan)
    assert len(plans) == 73
    write_json(ROOT / 'data/local/elite-monaspa-capture-preflight.json', dict(
        passed=True, subject=subject, matchups=len(plans), manifest=str(manifest),
        costBasis='Corrected Imperial purchase costs per physical unit; Elite Monaspa costs 60 food and 45 gold.',
        plans=plans))
