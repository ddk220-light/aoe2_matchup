"""Choose ten Elite Obuch Shorts from recorded outcomes and persisted unit costs."""
import argparse,json,sqlite3
from pathlib import Path
from overlay.battle_end import terminal_row
LAB=Path('aoe2x/js_simulation/calibration/lab').resolve()
OUT=LAB/'shorts/elite-obuch-selected-10'

def main():
    p=argparse.ArgumentParser();p.add_argument('--early',action='store_true');a=p.parse_args()
    state=json.loads((LAB/'campaigns/elite-obuch-final-overlays/status.json').read_text())
    ...
    for j in state['jobs']:
        ...
        inventory.append(dict(subject='elite-obuch',jobId=j['jobId'],unit=s['label'],civ=s['civ'],cost=sum(json.loads(Path('data/recording-costs.json').read_text())['units'][s['civ']+'|'+s['slug']]['effectiveCost'].values()),category=stats.get('unit_class_name','Special'),ranged=s['ranged'],winner=winner,winnerHp=sum(u['hp'] for u in last['sides'][str(winner)]) if winner else 0,run=str(run)))
    ...
    assert all(x['unit']!='Elite Obuch' for x in selected)
