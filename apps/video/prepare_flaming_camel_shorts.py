"""Choose ten Flaming Camel Shorts from recorded outcomes and persisted unit costs."""
import argparse,json,sqlite3
from pathlib import Path
from overlay.battle_end import terminal_row
LAB=Path('aoe2x/js_simulation/calibration/lab').resolve()
OUT=LAB/'shorts/flaming-camel-selected-10'

def main():
    p=argparse.ArgumentParser();p.add_argument('--early',action='store_true');a=p.parse_args()
    state=json.loads((LAB/'campaigns/flaming-camel-all-unique-overlays/status.json').read_text())
    ...
    for j in state['jobs']:
        ...
        inventory.append(dict(subject='flaming-camel',jobId=j['jobId'],unit=s['label'],civ=s['civ'],cost=s['weightedCost'],category=stats.get('unit_class_name','Special'),ranged=s['ranged'],winner=winner,winnerHp=sum(u['hp'] for u in last['sides'][str(winner)]) if winner else 0,run=str(run)))
    ...
    for name,reason in [('Missionary','Conversion attempts against a suicide army'),('Elite War Elephant','Explosion bonuses against a dense elephant army'),('Elite Tiger Cavalry','Explosions versus kill-based growth'),('Houfnice','A suicide charge against long-range siege'),('Elite Teutonic Knight','Infantry armor against explosions'),('Elite Cataphract','Armored cavalry facing a suicide charge'),('Elite Ghulam','Pass-through infantry attacks against camels')]:
    ...
    assert all(x['unit']!='Flaming Camel' for x in selected)
    notes.append('Flaming Camel self-match excluded; an eligible elephant matchup replaces that special slot.')
