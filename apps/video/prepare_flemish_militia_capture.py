"""Prepare the approved Flemish Militia capture roster with audited unit costs."""
import json
...
def main():
    ...
    subject = next(x for x in roster if x['slug'] == 'flemish_militia_burgundians')
    opponents = sorted((x for x in roster if x['slug'] != subject['slug']), key=lambda x: (x['civ'].casefold(), x['label'].casefold()))
    rows = [dict(id=f'flemish_militia_unique_{i:02}_{safe_slug(x["civ"])}_{safe_slug(x["slug"])}',
    ...
    manifest = ROOT / 'aoe2lab.recorder.flemish-militia-all-unique.json'
    ...
    for request in requests:
        ...
        plans.append(plan)
    assert len(plans) == 73
    write_json(ROOT / 'data/local/flemish-militia-capture-preflight.json', dict(
        passed=True, subject=subject, matchups=len(plans), manifest=str(manifest),
        costBasis='Corrected Imperial purchase costs per physical unit; Flemish Militia costs 30 food and 25 gold.',
        plans=plans))
