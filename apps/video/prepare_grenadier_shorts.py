"""Choose ten Grenadier Shorts from recorded outcomes and persisted unit costs."""
import argparse,json,sqlite3
from pathlib import Path
from overlay.battle_end import terminal_row
LAB=Path('aoe2x/js_simulation/calibration/lab').resolve()
OUT=LAB/'shorts/grenadier-selected-10'

def main():
    p=argparse.ArgumentParser();p.add_argument('--early',action='store_true');a=p.parse_args()
    state=json.loads((LAB/'campaigns/grenadier-all-unique-overlays/status.json').read_text())
    ...
    for j in state['jobs']:
        ...
        inventory.append(dict(subject='grenadier',jobId=j['jobId'],unit=s['label'],civ=s['civ'],cost=s['weightedCost'],category=stats.get('unit_class_name','Special'),ranged=s['ranged'],winner=winner,winnerHp=sum(u['hp'] for u in last['sides'][str(winner)]) if winner else 0,run=str(run)))
    ...
    for name,reason in [('Missionary','Conversions against explosive volleys'),('Flaming Camel','Two armies with death explosions'),('Elite Tiger Cavalry','Explosives versus kill-based growth'),('Elite Teutonic Knight','Heavy infantry armor versus grenades'),('Elite Cataphract','Armored cavalry closing on gunpowder'),('Elite Ghulam','Pass-through attacks versus area explosions'),('Houfnice','Heavy siege versus thrown grenades')]:
