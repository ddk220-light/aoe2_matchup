"""Choose ten Elite Ibirapema Warrior Shorts from recorded outcomes and persisted unit costs."""
import argparse,json,sqlite3
from pathlib import Path
from overlay.battle_end import terminal_row
LAB=Path('aoe2x/js_simulation/calibration/lab').resolve()
OUT=LAB/'shorts/ibirapema-warrior-selected-10'

def main():
    p=argparse.ArgumentParser();p.add_argument('--early',action='store_true');a=p.parse_args()
    state=json.loads((LAB/'campaigns/ibirapema-warrior-all-unique-overlays/status.json').read_text())
    ...
    for j in state['jobs']:
        ...
        inventory.append(dict(subject='ibirapema-warrior',jobId=j['jobId'],unit=s['label'],civ=s['civ'],cost=s['weightedCost'],category=stats.get('unit_class_name','Special'),ranged=s['ranged'],winner=winner,winnerHp=sum(u['hp'] for u in last['sides'][str(winner)]) if winner else 0,run=str(run)))
    ...
    for name,reason in [('Missionary','Conversion against heavy infantry'),('Flaming Camel','Explosive charge against infantry'),('Houfnice','Siege against advancing infantry'),('Elite Tiger Cavalry','Kill-based growth against a frontal area attack'),('Elite War Elephant','High-HP cavalry against heavy infantry'),('Elite Teutonic Knight','Heavy infantry duel')]:
    ...
    assert all(x['unit']!='Elite Ibirapema Warrior' for x in selected)
