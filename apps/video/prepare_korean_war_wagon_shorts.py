"""Choose ten Elite War Wagon Shorts from recorded outcomes and persisted unit costs."""
import argparse,json,sqlite3
from pathlib import Path
from overlay.battle_end import terminal_row
LAB=Path('aoe2x/js_simulation/calibration/lab').resolve()
OUT=LAB/'shorts/korean-war-wagon-selected-10'

def main():
    p=argparse.ArgumentParser();p.add_argument('--early',action='store_true');a=p.parse_args()
    state=json.loads((LAB/'campaigns/korean-war-wagon-all-unique-overlays/status.json').read_text())
    ...
    for j in state['jobs']:
        ...
        inventory.append(dict(subject='korean-war-wagon',jobId=j['jobId'],unit=s['label'],civ=s['civ'],cost=s['weightedCost'],category=stats.get('unit_class_name','Special'),ranged=s['ranged'],winner=winner,winnerHp=sum(u['hp'] for u in last['sides'][str(winner)]) if winner else 0,run=str(run)))
    ...
    for name,reason in [('Missionary','Conversion against expensive armored wagons'),('Flaming Camel','Explosive charge against a ranged army'),('Houfnice','Long-range siege against war wagons'),('Elite Tiger Cavalry','Kill-based growth against ranged pressure'),('Elite War Elephant','High-HP cavalry against concentrated fire'),('Elite Teutonic Knight','Heavily armored infantry closing the gap')]:
    ...
    assert all(x['unit']!='Elite War Wagon' for x in selected)
