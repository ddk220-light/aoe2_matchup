"""Choose ten Missionary Shorts from recorded outcomes and persisted unit costs."""
import argparse,json,sqlite3
from pathlib import Path
from overlay.battle_end import terminal_row
LAB=Path('aoe2x/js_simulation/calibration/lab').resolve()
OUT=LAB/'shorts/missionary-selected-10'

def main():
    p=argparse.ArgumentParser();p.add_argument('--early',action='store_true');a=p.parse_args()
    state=json.loads((LAB/'campaigns/missionary-all-unique-overlays/status.json').read_text())
    ...
    for j in state['jobs']:
        ...
        inventory.append(dict(subject='missionary',jobId=j['jobId'],unit=s['label'],civ=s['civ'],cost=s['weightedCost'],category=stats.get('unit_class_name','Special'),ranged=s['ranged'],winner=winner,winnerHp=sum(u['hp'] for u in last['sides'][str(winner)]) if winner else 0,run=str(run)))
    ...
    for name,reason in [('Flaming Camel','Conversion attempts against a suicide charge'),('Houfnice','Converting long-range siege'),('Elite War Elephant','Turning a high-value elephant into an ally'),('Elite Tiger Cavalry','Conversion against kill-based growth'),('Elite Teutonic Knight','Armored infantry closing on mounted monks'),('Elite Eagle Warrior','Conversion resistance and speed'),('Elite Magyar Huszar','Fast light cavalry closing the gap')]:
    ...
    assert 'Flaming Camel' in {x['unit'] for x in selected}
    assert all(x['unit']!='Missionary' for x in selected)
    notes.append('Missionary self-match excluded. Flaming Camel and siege included among the interesting matchups; missing cost categories are filled from recorded outcomes.')
