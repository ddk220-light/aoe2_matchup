...
OUT=LAB/'ibirapema-warrior-video-production'
CHANNEL={'id':'UCKYN-pN4AZ3w4LpRxcdSciA','handle':'@aoe2matchup'}
URL='https://aoe2matchup.com/?civ1=Tupi&unit1=elite_ibirapema_warrior_tupi&age1=Imperial'
...
def prepare_uploads(include_full=True):
    full=LAB/'compilations/ibirapema-warrior-unique-units/final'
    if include_full:
        media=json.loads((full/'manifest.json').read_text());assert media['fullDecode']=='passed' and media['matchups']==73
    selected=json.loads((LAB/'shorts/ibirapema-warrior-selected-10/selection.json').read_text())['items']
    items=[]
    def add(key,video,title,desc,kind):
        ...
        if not preparation.exists():write_json(preparation,{'uploadAuthorized':True,'targetChannelVerified':True,'targetChannel':CHANNEL,'video':str(video),'stateKey':key,'uploadScope':'Elite Ibirapema Warrior media prepared locally. Upload execution requires upload-approval.json recording existing user authorization.'})
        write_json(directory/'youtube-proposed-settings.json',{'uploadAuthorized':True,'channel':CHANNEL['handle'],'title':title,'descriptionFile':str(description),'categoryId':'20','defaultLanguage':'en','defaultAudioLanguage':'en','privacyStatus':'private','selfDeclaredMadeForKids':False,'containsSyntheticMedia':kind=='full','license':'youtube','embeddable':True,'tags':['AoE2','Age of Empires II Definitive Edition','Elite Ibirapema Warrior','Tupi','unit counters','equal resources','battle simulation','RTS'],'thumbnail':str(ROOT/'apps/video/intro/thumbnails'/('ibirapema-warrior-long.jpg' if kind=='full' else 'ibirapema-warrior-shorts.jpg'))})
        items.append({'key':key,'preparation':str(preparation),'title':title,'kind':kind,'subject':'Elite Ibirapema Warrior'})
    if include_full:add('ibirapema-warrior-full',Path(media['output']),'Elite Ibirapema Warrior vs 73 Unique-Unit Matchups | AoE2 DE',(full/'youtube-description.txt').read_text(encoding='utf-8'),'full')
    for x in selected:
        ...
        winner='Elite Ibirapema Warrior' if x['winner']==2 else x['unit'] if x['winner']==3 else 'Mutual elimination'
        desc=f"Elite Ibirapema Warrior vs {x['unit']} in Age of Empires II: Definitive Edition.\n\nEqual resources, maximum 27 units per main army. "

        if plan['side3']['ranged']: desc+='Ranged units get a small front line of hussars. '
        desc+='Live per-unit HP overlays and original game audio. Whole-unit rounding applies.\n\n'
        desc+=f"Winner: {winner}. Remaining HP: {round(x['winnerHp'])}."
        desc+=f'\n\nTry your own matchup:\n{URL}\n\n#Shorts #AoE2 #AoE2DE #IbirapemaWarrior #Tupi #RTS #BattleSimulation #UnitCounters #StrategyGaming\n'
        add(f"ibirapema-warrior-short-{x['number']:02}",directory/'short.mp4',f"Elite Ibirapema Warrior vs {x['unit']} | AoE2 DE #Shorts",desc,'short')
    batch=LAB/'youtube-batch-ibirapema-warrior';batch.mkdir(exist_ok=True)
    manifest=batch/('manifest.json' if include_full else 'shorts-manifest.json')
    write_json(manifest,{'authorized':True,'title':'Elite Ibirapema Warrior uploads','targetChannel':CHANNEL,'items':items})
    return manifest
...
def main():
    ...
    try:
        ...
        while True:
            ...
            overlay_status=LAB/'campaigns/ibirapema-warrior-all-unique-overlays/status.json'
            if not overlay_status.exists():time.sleep(30);continue
            overlays=json.loads(overlay_status.read_text())
            ...
            if overlays['state']=='NEEDS_ATTENTION':
                ...
                time.sleep(2)
                call('apps/video/render_campaign_overlays.py','--manifest',ROOT/'aoe2lab.recorder.ibirapema-warrior-all-unique.toml','--output',LAB/'campaigns/ibirapema-warrior-all-unique-overlays','--workers','1','--recording-status',LAB/'campaigns/ibirapema-warrior-all-unique/status.json',log_name='overlay-recovery.log')
                retried=json.loads((LAB/'campaigns/ibirapema-warrior-all-unique-overlays/status.json').read_text())
                ...
                break
            time.sleep(30)
        assert (OUT/'intro-ready.json').exists(), 'Intro music and visual/audio QA must finish before assembly'
        save('SELECTING_SHORTS');call('apps/video/prepare_ibirapema_warrior_shorts.py')
        intro=LAB/'compilations/ibirapema-warrior-unique-units/intro-v1/ibirapema-warrior-intro.mp4'
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            tasks=[]
            if not (LAB/'compilations/ibirapema-warrior-unique-units/final/manifest.json').exists():tasks.append(pool.submit(call,'apps/video/build_ibirapema_warrior_compilation.py'))
            tasks.append(pool.submit(call,'apps/video/render_selected_shorts.py','--selection',LAB/'shorts/ibirapema-warrior-selected-10/selection.json','--workers','1'))
