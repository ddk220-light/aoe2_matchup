...
OUT=LAB/'korean-fire-lancer-video-production'
CHANNEL={'id':'UCKYN-pN4AZ3w4LpRxcdSciA','handle':'@aoe2matchup'}
URL='https://aoe2matchup.com/?civ1=Koreans&unit1=elite_fire_lancer&age1=Imperial'
...
def prepare_uploads(include_full=True):
    full=LAB/'compilations/korean-fire-lancer-unique-units/final'
    if include_full:
        media=json.loads((full/'manifest.json').read_text());assert media['fullDecode']=='passed' and media['matchups']==74
    selected=json.loads((LAB/'shorts/korean-fire-lancer-selected-10/selection.json').read_text())['items']
    items=[]
    def add(key,video,title,desc,kind):
        ...
        if not preparation.exists():write_json(preparation,{'uploadAuthorized':True,'targetChannelVerified':True,'targetChannel':CHANNEL,'video':str(video),'stateKey':key,'uploadScope':'Elite Fire Lancer media prepared locally. Upload execution requires upload-approval.json recording existing user authorization.'})
        write_json(directory/'youtube-proposed-settings.json',{'uploadAuthorized':True,'channel':CHANNEL['handle'],'title':title,'descriptionFile':str(description),'categoryId':'20','defaultLanguage':'en','defaultAudioLanguage':'en','privacyStatus':'private','selfDeclaredMadeForKids':False,'containsSyntheticMedia':kind=='full','license':'youtube','embeddable':True,'tags':['AoE2','Age of Empires II Definitive Edition','Elite Fire Lancer','Koreans','unit counters','equal resources','battle simulation','RTS'],'thumbnail':str(ROOT/'apps/video/intro/thumbnails'/('korean-fire-lancer-long.jpg' if kind=='full' else 'korean-fire-lancer-shorts.jpg'))})
        items.append({'key':key,'preparation':str(preparation),'title':title,'kind':kind,'subject':'Elite Fire Lancer'})
    if include_full:add('korean-fire-lancer-full',Path(media['output']),'Elite Fire Lancer vs 74 Unique-Unit Matchups | AoE2 DE',(full/'youtube-description.txt').read_text(encoding='utf-8'),'full')
    for x in selected:
        ...
        winner='Elite Fire Lancer' if x['winner']==2 else x['unit'] if x['winner']==3 else 'Mutual elimination'
        desc=f"Elite Fire Lancer vs {x['unit']} in Age of Empires II: Definitive Edition.\n\nEqual resources, maximum 27 units per main army. "
        ...
        desc+=f'\n\nTry your own matchup:\n{URL}\n\n#Shorts #AoE2 #AoE2DE #FireLancer #Koreans #RTS #BattleSimulation #UnitCounters #StrategyGaming\n'
        add(f"korean-fire-lancer-short-{x['number']:02}",directory/'short.mp4',f"Elite Fire Lancer vs {x['unit']} | AoE2 DE #Shorts",desc,'short')
    batch=LAB/'youtube-batch-korean-fire-lancer';batch.mkdir(exist_ok=True)
    manifest=batch/('manifest.json' if include_full else 'shorts-manifest.json')
    write_json(manifest,{'authorized':True,'title':'Elite Fire Lancer uploads','targetChannel':CHANNEL,'items':items})
    return manifest
...
def main():
    ...
    try:
        ...
        while True:
            ...
            overlay_status=LAB/'campaigns/korean-fire-lancer-all-unique-overlays/status.json'
            ...
            if overlays['state']=='NEEDS_ATTENTION':
                ...
                time.sleep(2)
                call('apps/video/render_campaign_overlays.py','--manifest',ROOT/'aoe2lab.recorder.korean-fire-lancer-all-unique.toml','--output',LAB/'campaigns/korean-fire-lancer-all-unique-overlays','--workers','1','--recording-status',LAB/'campaigns/korean-fire-lancer-all-unique/status.json',log_name='overlay-recovery.log')
                retried=json.loads((LAB/'campaigns/korean-fire-lancer-all-unique-overlays/status.json').read_text())
                ...
                break
            time.sleep(30)
        save('SELECTING_SHORTS');call('apps/video/prepare_korean_fire_lancer_shorts.py')
        intro=LAB/'compilations/korean-fire-lancer-unique-units/intro-v1/korean-fire-lancer-intro.mp4'
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            tasks=[]
            if not (LAB/'compilations/korean-fire-lancer-unique-units/final/manifest.json').exists():tasks.append(pool.submit(call,'apps/video/build_korean_fire_lancer_compilation.py'))
            tasks.append(pool.submit(call,'apps/video/render_selected_shorts.py','--selection',LAB/'shorts/korean-fire-lancer-selected-10/selection.json','--workers','1'))
