...
OUT=LAB/'liao-video-production'
CHANNEL={'id':'UCKYN-pN4AZ3w4LpRxcdSciA','handle':'@aoe2matchup'}
URL='https://aoe2matchup.com/?civ1=Khitans&unit1=elite_liao_dao_khitans&age1=Imperial'
...
def prepare_uploads(include_full=True):
    full=LAB/'compilations/liao-dao-unique-units/final'
    if include_full:
        media=json.loads((full/'manifest.json').read_text());assert media['fullDecode']=='passed' and media['matchups']==73
    selected=json.loads((LAB/'shorts/liao-selected-10/selection.json').read_text())['items']
    items=[]
    def add(key,video,title,desc,kind):
        ...
        if not preparation.exists():write_json(preparation,{'uploadAuthorized':True,'targetChannelVerified':True,'targetChannel':CHANNEL,'video':str(video),'stateKey':key,'uploadScope':'User requested the Liao Dao full video and ten Shorts uploaded to YouTube.'})
        write_json(directory/'youtube-proposed-settings.json',{'uploadAuthorized':True,'channel':CHANNEL['handle'],'title':title,'descriptionFile':str(description),'categoryId':'20','defaultLanguage':'en','defaultAudioLanguage':'en','privacyStatus':'private','selfDeclaredMadeForKids':False,'containsSyntheticMedia':kind=='full','license':'youtube','embeddable':True,'tags':['AoE2','Age of Empires II Definitive Edition','Liao Dao','Khitans','unit counters','equal resources','battle simulation','RTS'],'thumbnail':str(ROOT/'apps/video/intro/thumbnails'/('liao-long.jpg' if kind=='full' else 'liao-shorts.jpg'))})
        items.append({'key':key,'preparation':str(preparation),'title':title,'kind':kind,'subject':'Elite Liao Dao'})
    if include_full:add('liao-full',Path(media['output']),'Elite Liao Dao vs 73 Unique-Unit Matchups | AoE2 DE',(full/'youtube-description.txt').read_text(encoding='utf-8'),'full')
    for x in selected:
        ...
        winner='Elite Liao Dao' if x['winner']==2 else x['unit'] if x['winner']==3 else 'No decisive winner'
        desc=f"Elite Liao Dao vs {x['unit']} in Age of Empires II: Definitive Edition.\n\nEqual resources, maximum 27 units per main army. "
        ...
        desc+=f'\n\nTry your own matchup:\n{URL}\n\n#Shorts #AoE2 #AoE2DE #LiaoDao #Khitans #RTS #BattleSimulation #UnitCounters #StrategyGaming\n'
        add(f"liao-short-{x['number']:02}",directory/'short.mp4',f"Elite Liao Dao vs {x['unit']} | AoE2 DE #Shorts",desc,'short')
    batch=LAB/'youtube-batch-liao';batch.mkdir(exist_ok=True)
    manifest=batch/('manifest.json' if include_full else 'shorts-manifest.json')
    write_json(manifest,{'authorized':True,'title':'Elite Liao Dao uploads','targetChannel':CHANNEL,'items':items})
    return manifest
...
def main():
    ...
    try:
        ...
        while True:
            overlays=json.loads((LAB/'campaigns/liao-dao-all-unique-overlays/status.json').read_text())
            ...
            if overlays['state']=='NEEDS_ATTENTION':
                ...
                time.sleep(2)
                call('apps/video/render_campaign_overlays.py','--manifest',ROOT/'aoe2lab.recorder.liao-dao-all-unique.toml','--output',LAB/'campaigns/liao-dao-all-unique-overlays','--workers','3',log_name='overlay-recovery.log')
                retried=json.loads((LAB/'campaigns/liao-dao-all-unique-overlays/status.json').read_text())
                ...
                break
            time.sleep(30)
        save('SELECTING_SHORTS');call('apps/video/prepare_liao_shorts.py')
        intro=LAB/'compilations/liao-dao-unique-units/intro-v1/liao-dao-intro.mp4'
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=2) as pool:
            tasks=[]
            if not (LAB/'compilations/liao-dao-unique-units/final/manifest.json').exists():tasks.append(pool.submit(call,'apps/video/build_liao_compilation.py'))
            tasks.append(pool.submit(call,'apps/video/render_selected_shorts.py','--selection',LAB/'shorts/liao-selected-10/selection.json','--workers','2'))
