...
OUT=LAB/'mounted-trebuchet-video-production'
CHANNEL={'id':'UCKYN-pN4AZ3w4LpRxcdSciA','handle':'@aoe2matchup'}
URL='https://aoe2matchup.com/?civ1=Khitans&unit1=mounted_trebuchet_khitans&age1=Imperial'
...
def prepare_uploads(include_full=True, include_shorts=True):
    full=LAB/'compilations/mounted-trebuchet-unique-units/final'
    if include_full:
        media=json.loads((full/'manifest.json').read_text());assert media['fullDecode']=='passed' and media['matchups']==33
    selected=json.loads((LAB/'shorts/mounted-trebuchet-selected-10/selection.json').read_text())['items'] if include_shorts else []
    items=[]
    def add(key,video,title,desc,kind):
        ...
        if not preparation.exists():write_json(preparation,{'uploadAuthorized':True,'targetChannelVerified':True,'targetChannel':CHANNEL,'video':str(video),'stateKey':key,'uploadScope':'Mounted Trebuchet media prepared locally. Upload execution requires upload-approval.json recording existing user authorization.'})
        write_json(directory/'youtube-proposed-settings.json',{'uploadAuthorized':True,'channel':CHANNEL['handle'],'title':title,'descriptionFile':str(description),'categoryId':'20','defaultLanguage':'en','defaultAudioLanguage':'en','privacyStatus':'private','selfDeclaredMadeForKids':False,'containsSyntheticMedia':kind=='full','license':'youtube','embeddable':True,'tags':['AoE2','Age of Empires II Definitive Edition','Mounted Trebuchet','Khitans','unit counters','equal resources','battle simulation','RTS'],'thumbnail':str(ROOT/'apps/video/intro/thumbnails'/('mounted-trebuchet-long.jpg' if kind=='full' else 'mounted-trebuchet-shorts.jpg'))})
        items.append({'key':key,'preparation':str(preparation),'title':title,'kind':kind,'subject':'Mounted Trebuchet'})
    if include_full:add('mounted-trebuchet-full',Path(media['output']),'Mounted Trebuchet vs 33 Ranged Unique-Unit Matchups | AoE2 DE',(full/'youtube-description.txt').read_text(encoding='utf-8'),'full')
    for x in selected:
        ...
        winner='Mounted Trebuchet' if x['winner']==2 else x['unit'] if x['winner']==3 else 'Mutual elimination'
        desc=f"Mounted Trebuchet vs {x['unit']} in Age of Empires II: Definitive Edition.\n\nEqual resources, maximum 27 units per main army. "
        desc+='Ranged opponents only; no extra frontline buffer. '
        ...
        desc+=f'\n\nTry your own matchup:\n{URL}\n\n#Shorts #AoE2 #AoE2DE #MountedTrebuchet #Khitans #RTS #BattleSimulation #UnitCounters #StrategyGaming\n'
        add(f"mounted-trebuchet-short-{x['number']:02}",directory/'short.mp4',f"Mounted Trebuchet vs {x['unit']} | AoE2 DE #Shorts",desc,'short')
    batch=LAB/'youtube-batch-mounted-trebuchet';batch.mkdir(exist_ok=True)
    manifest=batch/('full-manifest.json' if not include_shorts else 'manifest.json' if include_full else 'shorts-manifest.json')
    write_json(manifest,{'authorized':True,'title':'Mounted Trebuchet uploads','targetChannel':CHANNEL,'items':items})
    return manifest
...
def main():
    ...
    try:
        ...
        while True:
            ...
            overlays=json.loads((LAB/'campaigns/mounted-trebuchet-all-unique-overlays/status.json').read_text())
            ...
            if overlays['state']=='NEEDS_ATTENTION':
                ...
                time.sleep(2)
                call('apps/video/render_campaign_overlays.py','--manifest',ROOT/'aoe2lab.recorder.mounted-trebuchet-all-unique.toml','--output',LAB/'campaigns/mounted-trebuchet-all-unique-overlays','--workers','1','--recording-status',LAB/'campaigns/mounted-trebuchet-all-unique/status.json',log_name='overlay-recovery.log')
                retried=json.loads((LAB/'campaigns/mounted-trebuchet-all-unique-overlays/status.json').read_text())
                ...
                break
            time.sleep(30)
        save('SELECTING_SHORTS');call('apps/video/prepare_mounted_trebuchet_shorts.py')
        intro=LAB/'compilations/mounted-trebuchet-unique-units/intro-v1/mounted-trebuchet-intro.mp4'
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            tasks=[]
            if not (LAB/'compilations/mounted-trebuchet-unique-units/final/manifest.json').exists():tasks.append(pool.submit(call,'apps/video/build_mounted_trebuchet_compilation.py'))
            tasks.append(pool.submit(call,'apps/video/render_selected_shorts.py','--selection',LAB/'shorts/mounted-trebuchet-selected-10/selection.json','--workers','1'))
