const rankingsData = import("./shared/page-data.js").then(m => m.createPageData());
/* ==========================================================================
...
async function getStatChain(refUnitId) {
    ...
    try {
        const data = await (await rankingsData).detail(`/api/ref/stat-chain/${refUnitId}`);
...
async function selectLine(slug) {
    ...
    let data;
    try { data = await (await rankingsData).select(`/api/ref/unit-line/${slug}`); }
    catch (error) {
        if (error.name === "AbortError" || currentLine !== slug) return;
        document.getElementById("tableContainer").textContent = "Could not load rankings. Please select a tab to retry.";
        return;
    }
    if (!data || currentLine !== slug) return;
    currentData = data;
