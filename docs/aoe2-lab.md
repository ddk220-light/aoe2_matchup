...
# A repeatable queue of requested matchups (one entry in the spike example):
.\scripts\aoe2lab.ps1 batch .\aoe2lab.recorder.example.toml --phase recorder
```

Each successful repeat is stored under the configured artifacts directory:
...
runs/<job_id>/
  ...
  live/run_001/
    ...
    grpc_logger.log
    raw recordings/
      <matchup>.mov                  raw screen recording; no added overlays
      <matchup>.frames.bin           original gRPC stream for offline redecoding
      <matchup>.meta.json
      <matchup>.END
      <matchup>.hp.json              decoded aggregate HP + raw-video timing
```
...
attempts retain diagnostic evidence. The mode is also available as
`run --mode recorder` when engine simulation and comparison are wanted too.

Before recording, keep AoE2 maximized/fullscreen on the primary display in the
...
enabled. The builder previously overwrote that target from army centroids; it
now preserves it for every count/ranged combination. Lab validation rejects a
generated scenario whose camera differs from the golden.
