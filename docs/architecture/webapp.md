...
| `/api/ref/unit-line/<line_slug>` | GET | `aoe2_reference.db` + `derived_data.db` (`battle_scores`) + `pool_scores.db` + `patches.db` (current build) + `battle_scores.json` (legacy fallback) | `rankings.js` |
...
| Server-side civ list | `aoe2_reference.db` via `_valid_civs()` in `app.py` | Authoritative civ set for validation and the sitemap | every validated route | n/a — derived from the DB. Note: `ORIGINAL_13_CIVS` in `app.py` (line 682) is **dead code**, defined but never referenced; the live copy of that list is in `analysis/config_constants.py` |
...
JSON artifacts read at runtime: `battle_scores.json` (loaded once at import; legacy fallback scores for lines not covered by `derived_data.db`), `civ_power_units/<build>.json` with `civ_power_units.json` fallback, and `civ_top_units.json`. `webapp/matchup_db.db` (the 200+ MB raw sim cache) and `battle_cache.json` are **not** read by any route — they feed the offline derive jobs only.
...
| Old rule | Status now |
|---|---|
| `UNIT_LINES` duplicated in `app.py` + `compute_battle_scores.py` | **Retired** — single source `webapp/unit_lines.py`. Residual drift: the supplementary `*_LINE_SLUGS` sets are still defined separately in `app.py` (lines 1024–1028) and `compute_battle_scores.py` (585+), and disagree on mangonel (`SIEGE_LINE_SLUGS` vs `HIDDEN_LINE_SLUGS`) — documented as a TODO in `unit_lines.py` |
...
| `ENABLED_CIVS` in 2 templates matching `ORIGINAL_13_CIVS` | **Changed** — one copy in `constants.js`; must match the civ set in `aoe2_reference.db` (the server validates against the DB, not any Python list). `ORIGINAL_13_CIVS` in `app.py` is unused |
...
| Battle scores stale after sim changes | **Still true, different artifacts** — regenerate `derived_data.db`, `pool_scores.db`, `civ_power_units/` via the patch pipeline ([derived-data.md](derived-data.md)), not just `battle_scores.json` |
