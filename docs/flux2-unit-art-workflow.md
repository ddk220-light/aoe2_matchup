...
- **Keep the single-figure guard even when short.** "One subject, one single view" alone isn't always enough — the Guecha Warrior short prompt duplicated into two figures. End every prompt (short or long) with "Output EXACTLY ONE single figure — no duplicate, no second copy, no grid, no extra person in the background."
