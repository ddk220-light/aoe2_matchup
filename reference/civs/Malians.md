# Malians

**Focus:** Infantry  
**Sources:** Fandom wiki, SiegeEngineers/aoe2techtree  
**Generated:** 2026-04-12

## Civilization Bonuses

- Buildings cost -15% wood (except Farms and starting Town Center).
- Barracks units have +1/+2/+3 pierce armor in the Feudal/Castle/Imperial Age.
- Villagers drop off .This bonus has three implications:
- Gold miners and Oyster gatherers work 10% faster.
- Gold Mines and Oysters have .
- Gold miners and Oyster gatherers have +10% carry capacity.

## Team Bonus

Universities work +80% faster.

## Unique Technologies

| Tech | Effect |
|------|--------|
| Tigui | Town Centers fire +8 arrows always |
| Farimba | Cavalry units +5 attack |

## Unique Units

### Gbeto

| Stat | Regular | Elite |
|------|---------|-------|
| HP | 40.0 | 50.0 |
| Attack | 10.0 | 13.0 |
| Melee Armor | 0.0 | 0.0 |
| Pierce Armor | 0.0 | 0.0 |
| Speed | 1.25 | 1.25 |
| Range | 5.0 | 6.0 |
| Reload Time | 2.0 | 2.0 |
| Cost Food | 50.0 | 50.0 |
| Cost Wood | 0.0 | 0.0 |
| Cost Gold | 40.0 | 40.0 |

_Full stat comparison: see `reference/units/unique/Gbeto.md`_

## Standard Units Available

Arbalester, Battering Ram, Bombard Cannon, Camel Rider, Capped Ram, Cavalier, Cavalry Archer, Champion, Crossbowman, Elite Skirmisher, Hand Cannoneer, Heavy Camel Rider, Heavy Cavalry Archer, Knight, Light Cavalry, Long Swordsman, Mangonel, Pikeman, Scorpion, Siege Onager, Trebuchet

## Disabled Standard Units

_None — this civ has access to all widely-available standard units._

## DB Summary

- Total units in DB: 27
- Unique units: 2
- Standard units: 25
- Disabled standard units: 0
