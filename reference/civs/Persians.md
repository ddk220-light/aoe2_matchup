# Persians

**Focus:** Cavalry  
**Sources:** Fandom wiki, SiegeEngineers/aoe2techtree  
**Generated:** 2026-04-12

## Civilization Bonuses

- Start the game with +50 food, +50 wood.
- Town Centers and Docks have double hit points and work 5%/10%/15%/20% faster in the Dark/Feudal/Castle/Imperial Age.
- Parthian Tactics available in the Castle Age.
- Can build Caravanserais in the Imperial Age.

## Team Bonus

Knight line has +2 attack against Archers.

## Unique Technologies

| Tech | Effect |
|------|--------|
| Kamandaran | Archer-line cost changed from  to {{Resources |
| Citadels | Castles +4 attack, +3 against Rams, +3 against {{2class |

## Unique Units

### War Elephant

| Stat | Regular | Elite |
|------|---------|-------|
| HP | 450.0 | 600.0 |
| Attack | 15.0 | 20.0 |
| Melee Armor | 1.0 | 1.0 |
| Pierce Armor | 2.0 | 3.0 |
| Speed | 0.8 | 0.8 |
| Range | 0.0 | 0.0 |
| Reload Time | 2.0 | 2.0 |
| Cost Food | 170.0 | 170.0 |
| Cost Wood | 0.0 | 0.0 |
| Cost Gold | 85.0 | 85.0 |

_Full stat comparison: see `reference/units/unique/War_Elephant.md`_

## Standard Units Available

Battering Ram, Bombard Cannon, Camel Rider, Cavalry Archer, Crossbowman, Elite Skirmisher, Halberdier, Hand Cannoneer, Heavy Camel Rider, Heavy Cavalry Archer, Heavy Scorpion, Hussar, Knight, Light Cavalry, Long Swordsman, Mangonel, Onager, Pikeman, Savar, Scorpion, Siege Ram, Trebuchet

## Disabled Standard Units

_None — this civ has access to all widely-available standard units._

## DB Summary

- Total units in DB: 27
- Unique units: 2
- Standard units: 25
- Disabled standard units: 0
