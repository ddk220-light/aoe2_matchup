# Battle Elephant

**Type:** Standard  
**Available to:** All civs  
**Sources:** SiegeEngineers/aoe2techtree, Fandom wiki  
**Generated:** 2026-04-13

## Stats

| Stat | Castle | Imperial |
|------|---------|-------|
| HP | 230.0 | 250.0 |
| Attack | 6.0 | 12.0 |
| Melee Armor | 0.0 | 1.0 |
| Pierce Armor | 2.0 | 2.0 |
| Speed | 0.9 | 0.9 |
| Range | 4.0 | 0.0 |
| Reload Time | 2.0 | 2.0 |
| Cost Food | 60.0 | 100.0 |
| Cost Wood | 0.0 | 0.0 |
| Cost Gold | 80.0 | 70.0 |
| Pop Space | 1.0 | 1.0 |

## Special Effects

- **pass_through_count:** 1.0
- **hp_regen:** 30.0

## DB Comparison

| Field | External | Our DB (Castle) | Match |
|-------|----------|-----------------|-------|
| HP | 250.0 | 230.0 | ❌ |
| Attack | 12.0 | 6.0 | ❌ |
| Melee Armor | 1.0 | 0.0 | ❌ |
| Pierce Armor | 2.0 | 2.0 | ✅ |
| Speed | ⚠️ | 0.9 | ⚠️ |
| Range | ⚠️ | 4.0 | ⚠️ |
| Reload Time | 2.0 | 2.0 | ✅ |
| Cost Food | 75 | 60.0 | ❌ |
| Cost Wood | 0 | 0.0 | ✅ |
| Cost Gold | 70 | 80.0 | ❌ |

**⚠️ 5 mismatch(es) found — investigate.**

## Attack Bonuses

| Bonus | Armor Class |
|-------|-------------|
| -3 | Mounted Archers |

## Armor Classes (Vulnerability)

_Units with attack bonuses against these classes deal extra damage to this unit._

| Armor Class | Armor Value |
|-------------|-------------|
| War Elephants | 0 |
| Cavalry | 0 |
| Archers | 0 |
| Cavalry Archers | -4 |
| Leitis | 0 |

## Strengths & Weaknesses

**Strong vs:** building, foot archer
**Weak vs:** Pikemen, Kamayuk, Genoese Crossbowmen, Mameluke, Cavalry Archer, Monk, War Elephant, Scorpion, Flaming Camel

## Ability

_Wootz Steel (attack ignores armor, Dravidians only)_

## Technologies

| Stat | Technology (Effect) |
|------|---------------------|
| HP | Bloodlines (+20) |
| HP | Chatras (+100, Vietnamese only) |
| Attack | Forging (+1) |
| Attack | Iron Casting (+1) |
| Attack | Blast Furnace (+2) |
| Attack | Manipur Cavalry (+4 vs. Archers, Burmese only) |
| Attack | Tusk Swords (+3, Khmer only) |
| Armor | Scale Barding Armor (melee=+1, pierce=+1) |
| Armor | Chain Barding Armor (melee=+1, pierce=+1) |
| Armor | Plate Barding Armor (melee=+1, pierce=+2) |
| Armor | Howdah (+1 melee/+2 pierce between Dawn of the Dukes and Dynasties of India, +1 melee/+1 pierce after patch 5.7 and before Dawn of the Dukes, +1 melee/+2 pierce before patch 5.7, Burmese only) |
| Speed | Husbandry (+10%) |
| Conversion | Devotion (+1 min, +1 max) |
| Conversion | Faith (+4 min, +4 max) |
| Conversion | Heresy (die upon getting converted, Malay only) |
| Creation | Conscription (+33%) |

## Civilization Bonuses

**Unique to civ:**

| Stat | Civ (Bonus) |
|------|-------------|
| Attack | Bengalis (+2 vs. skirmishers) |
| Armor | Burmese (melee=+1, pierce=+1) |
| Speed | Khmer (+10%) |
| Conversion | Bengalis (+3 min, +1 max) |

**Team bonuses:**

| Stat | Civ (Bonus) |
|------|-------------|
| Attack | Wei (+2 vs. Siege Weapons) |
| Conversion | Teutons (+3 min, +1 max) |
| Creation | Gurjaras (+25%) |
| Creation | Huns (+20%) |
