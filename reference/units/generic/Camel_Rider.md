# Camel Rider

**Type:** Standard  
**Available to:** All civs  
**Sources:** SiegeEngineers/aoe2techtree, Fandom wiki  
**Generated:** 2026-04-13

## Stats

| Stat | Castle | Imperial |
|------|---------|-------|
| HP | 100.0 | 100.0 |
| Attack | 6.0 | 6.0 |
| Melee Armor | 0.0 | 0.0 |
| Pierce Armor | 0.0 | 0.0 |
| Speed | 1.45 | 1.45 |
| Range | 0.0 | 0.0 |
| Reload Time | 2.0 | 2.0 |
| Cost Food | 55.0 | 55.0 |
| Cost Wood | 0.0 | 0.0 |
| Cost Gold | 60.0 | 60.0 |
| Pop Space | 1.0 | 1.0 |

## Special Effects

- **pass_through_count:** 1.0

## DB Comparison

| Field | External | Our DB (Castle) | Match |
|-------|----------|-----------------|-------|
| HP | 100.0 | 100.0 | ✅ |
| Attack | ⚠️ | 6.0 | ⚠️ |
| Melee Armor | 0.0 | 0.0 | ✅ |
| Pierce Armor | 0.0 | 0.0 | ✅ |
| Speed | ⚠️ | 1.45 | ⚠️ |
| Range | ⚠️ | 0.0 | ⚠️ |
| Reload Time | 2.0 | 2.0 | ✅ |
| Cost Food | 325 | 55.0 | ❌ |
| Cost Wood | 0 | 0.0 | ✅ |
| Cost Gold | 360 | 60.0 | ❌ |

**⚠️ 2 mismatch(es) found — investigate.**

## Attack Bonuses

| Bonus | Armor Class |
|-------|-------------|
| +9 | Cavalry |
| +5 | Ships & Saboteurs |
| +5 | Camels |
| -3 | Mounted Archers |

## Armor Classes (Vulnerability)

_Units with attack bonuses against these classes deal extra damage to this unit._

| Armor Class | Armor Value |
|-------------|-------------|
| Camels | 0 |
| Leitis | 0 |
| Mounted Archers | -3 |

## Strengths & Weaknesses

**Strong vs:** Elite Skirmisher, siege weapon, mounted archer, cavalry
**Weak vs:** Infantry, foot archer, Cataphract, defensive structure

## Technologies

| Stat | Technology (Effect) |
|------|---------------------|
| HP | Bloodlines (+20) |
| Attack | Forging (+1, +2 for the Khitans) |
| Attack | Iron Casting (+1, +2 for the Khitans) |
| Attack | Blast Furnace (+2) |
| Attack | Farimba (+5, Malians only) |
| Armor | Scale Barding Armor (melee=+1, pierce=+1) |
| Armor | Chain Barding Armor (melee=+1, pierce=+1) |
| Armor | Plate Barding Armor (melee=+1, pierce=+2) |
| Armor | Frontier Guards (melee=+4, Gurjaras only) |
| Speed | Husbandry (+10%) |
| Conversion | Devotion (+1 min, +1 max) |
| Conversion | Faith (+4 min, +4 max) |
| Conversion | Heresy (die upon getting converted) |
| Creation | Conscription (+33%) |

## Civilization Bonuses

**Unique to civ:**

| Stat | Civ (Bonus) |
|------|-------------|
| HP | Saracens (+25%) |
| Attack | Gurjaras (+30%/+40% bonus damage in the Castle/Imperial Age) |
| Speed | Cumans (+10%/+15% in the Castle/Imperial Age) |

**Team bonuses:**

| Stat | Civ (Bonus) |
|------|-------------|
| Attack | Hindustanis (+2 attack against Standard Buildings) |
| Attack | Wei (+2 vs. Siege Weapons) |
| Conversion | Teutons (+3 min, +1 max) |
| Creation | Gurjaras (+25%) |
| Creation | Huns (+20%) |
