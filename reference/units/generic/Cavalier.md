# Cavalier

**Type:** Standard  
**Available to:** All civs  
**Sources:** SiegeEngineers/aoe2techtree, Fandom wiki  
**Generated:** 2026-04-12

## Stats

| Stat | Imperial | Elite |
|------|---------|-------|
| HP | 90.0 | — |
| Attack | 12.0 | — |
| Melee Armor | 4.0 | — |
| Pierce Armor | 3.0 | — |
| Speed | 1.35 | — |
| Range | 0.0 | — |
| Reload Time | 1.8 | — |
| Cost Food | 65.0 | — |
| Cost Wood | 0.0 | — |
| Cost Gold | 65.0 | — |
| Pop Space | 1.0 | — |

## Special Effects

- **pass_through_count:** 1.0

## DB Comparison

| Field | External | Our DB (Imperial) | Match |
|-------|----------|-----------------|-------|
| HP | 120.0 | 90.0 | ❌ |
| Attack | 12.0 | 12.0 | ✅ |
| Melee Armor | 2.0 | 4.0 | ❌ |
| Pierce Armor | 2.0 | 3.0 | ❌ |
| Speed | 1.35 | 1.35 | ✅ |
| Range | ⚠️ | 0.0 | ⚠️ |
| Reload Time | 1.8 | 1.8 | ✅ |
| Cost Food | 300 | 65.0 | ❌ |
| Cost Wood | 0 | 0.0 | ✅ |
| Cost Gold | 300 | 65.0 | ❌ |

**⚠️ 5 mismatch(es) found — investigate.**

## Attack Bonuses

| Bonus | Armor Class |
|-------|-------------|
| -3 | Mounted Archers |
| +1 | Infantry |

## Armor Classes (Vulnerability)

_Units with attack bonuses against these classes deal extra damage to this unit._

| Armor Class | Armor Value |
|-------------|-------------|
| Cavalry | 0 |
| Leitis | 0 |

## Strengths & Weaknesses

**Strong vs:** Infantry, Villager, siege weapon, light cavalry, Plumed Archer, Elite Skirmisher
**Weak vs:** Boyar, Halberdier, Kamayuk, Berserk, Genoese Crossbowmen, Teutonic Knight, Mameluke, Heavy Camel Rider, Monk, War Elephant, Elite Battle Elephant

## Technologies

| Stat | Technology (Effect) |
|------|---------------------|
| HP | Bloodlines (+20) |
| Attack | Forging (+1) |
| Attack | Iron Casting (+1) |
| Attack | Blast Furnace (+2) |
| Attack | Comitatenses (+5 charge every 20 secs, Romans only) |
| Attack | Farimba (+5, Malians only) |
| Attack | Manipur Cavalry (+4 vs. Archers, Burmese only) |
| Armor | Scale Barding Armor (melee=+1, pierce=+1) |
| Armor | Chain Barding Armor (melee=+1, pierce=+1) |
| Armor | Plate Barding Armor (melee=+1, pierce=+2) |
| Armor | Hauberk (melee=+1, pierce=+2, Sicilians only) |
| Speed | Husbandry (+10%) |
| Conversion | Devotion (+1 min, +1 max) |
| Conversion | Faith (+4 min, +4 max) |
| Conversion | Heresy (die upon getting converted) |
| Conversion | First Crusade (+4 min, +4 max, Sicilians only) |
| Creation | Conscription (+33%) |
| Creation | Chivalry (+40%, Franks only) |
| Creation | Comitatenses (+50%, Romans only) |

## Civilization Bonuses

**Unique to civ:**

| Stat | Civ (Bonus) |
|------|-------------|
| HP | Franks (+20%) |
| Attack | Lithuanians (+1 per Relic garrisoned, max +4) |
| Attack | Tatars (+20% from higher elevation) |
| Armor | Teutons (melee=+2) |
| Speed | Cumans (+15%) |
| Other | Burgundians (available in the Castle Age) |

**Team bonuses:**

| Stat | Civ (Bonus) |
|------|-------------|
| Attack | Persians (+2 vs. Archers) |
| Attack | Wei (+2 vs. siege) |
| Conversion | Teutons (+3 min, +1 max) |
| Creation | Huns (+20%) |
| LOS | Franks (+2) |
