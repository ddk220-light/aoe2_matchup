# Hand Cannoneer

**Type:** Standard  
**Available to:** All civs  
**Sources:** SiegeEngineers/aoe2techtree, Fandom wiki  
**Generated:** 2026-04-12

## Stats

| Stat | Imperial | Elite |
|------|---------|-------|
| HP | 40.0 | — |
| Attack | 17.0 | — |
| Melee Armor | 1.0 | — |
| Pierce Armor | 0.0 | — |
| Speed | 0.96 | — |
| Range | 7.0 | — |
| Reload Time | 3.45 | — |
| Cost Food | 45.0 | — |
| Cost Wood | 0.0 | — |
| Cost Gold | 50.0 | — |
| Pop Space | 1.0 | — |

## Special Effects

- **pass_through_count:** 1.0

## DB Comparison

| Field | External | Our DB (Imperial) | Match |
|-------|----------|-----------------|-------|
| HP | ⚠️ | 40.0 | ⚠️ |
| Attack | 17.0 | 17.0 | ✅ |
| Melee Armor | 1.0 | 1.0 | ✅ |
| Pierce Armor | 0.0 | 0.0 | ✅ |
| Speed | 0.96 | 0.96 | ✅ |
| Range | 7.0 | 7.0 | ✅ |
| Reload Time | 3.45 | 3.45 | ✅ |
| Cost Food | 45 | 45.0 | ✅ |
| Cost Wood | 25 | 0.0 | ❌ |
| Cost Gold | 50 | 50.0 | ✅ |

**⚠️ 1 mismatch(es) found — investigate.**

## Attack Bonuses

| Bonus | Armor Class |
|-------|-------------|
| +10 | Infantry |
| -10 | Condottieri |
| +2 | Rams |
| +1 | Spearmen |

## Armor Classes (Vulnerability)

_Units with attack bonuses against these classes deal extra damage to this unit._

| Armor Class | Armor Value |
|-------------|-------------|
| Archers | 0 |
| Gunpowder Units | 0 |
| Leitis | 0 |

## Strengths & Weaknesses

**Strong vs:** Infantry, Heavy Camel Rider
**Weak vs:** Elite Skirmisher, Scorpion, Onager, Genitour, Condottieri

## Technologies

| Stat | Technology (Effect) |
|------|---------------------|
| Armor | Padded Archer Armor (melee=+1, pierce=+1) |
| Armor | Leather Archer Armor (melee=+1, pierce=+1) |
| Armor | Ring Archer Armor (melee=+1, pierce=+2) |
| Speed | Wagenburg Tactics (+15%, Bohemians only) |
| Conversion | Devotion (+1 min, +1 max) |
| Conversion | Faith (+4 min, +4 max) |
| Conversion | Heresy (die upon getting converted) |
| Creation | Conscription (+33%) |
| Other | Arquebus (projectile speed +0.5 tiles/second, Portuguese only) |

## Civilization Bonuses

**Unique to civ:**

| Stat | Civ (Bonus) |
|------|-------------|
| HP | Turks (+25%) |
| Attack | Burgundians (+25%) |
| Attack | Tatars (+20% damage {+50% instead of +25%} from higher elevation) |
| Armor | Hindustanis (melee=+1, pierce=+1) |
| Other | Bohemians (Hand Cannoneers and Chemistry available in the Castle Age) |
| Other | Italians (Chemistry costs -33%) |
| Other | Turks (Chemistry is free) |

**Team bonuses:**

| Stat | Civ (Bonus) |
|------|-------------|
| Conversion | Teutons (+3 min, +1 max) |
| Creation | Britons (+10%) |
| Creation | Turks (+25%) |
| LOS | Jurchens (+2) |
| Other | Malians (Chemistry researches 80% faster) |
| Other | Portuguese (Chemistry researches 25% faster) |
