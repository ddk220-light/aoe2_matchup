# Pikeman

**Type:** Standard  
**Available to:** All civs  
**Sources:** SiegeEngineers/aoe2techtree, Fandom wiki  
**Generated:** 2026-04-13

## Stats

| Stat | Castle | Elite |
|------|---------|-------|
| HP | 55.0 | — |
| Attack | 4.0 | — |
| Melee Armor | 0.0 | — |
| Pierce Armor | 0.0 | — |
| Speed | 1.0 | — |
| Range | 0.0 | — |
| Reload Time | 3.0 | — |
| Cost Food | 35.0 | — |
| Cost Wood | 25.0 | — |
| Cost Gold | 0.0 | — |
| Pop Space | 1.0 | — |

## Special Effects

- **pass_through_count:** 1.0

## DB Comparison

| Field | External | Our DB (Castle) | Match |
|-------|----------|-----------------|-------|
| HP | 55.0 | 55.0 | ✅ |
| Attack | 4.0 | 4.0 | ✅ |
| Melee Armor | 0.0 | 0.0 | ✅ |
| Pierce Armor | 0.0 | 0.0 | ✅ |
| Speed | 1.0 | 1.0 | ✅ |
| Range | ⚠️ | 0.0 | ⚠️ |
| Reload Time | 3.0 | 3.0 | ✅ |
| Cost Food | 300 | 35.0 | ❌ |
| Cost Wood | 25 | 25.0 | ✅ |
| Cost Gold | 600 | 0.0 | ❌ |

**⚠️ 2 mismatch(es) found — investigate.**

## Attack Bonuses

| Bonus | Armor Class |
|-------|-------------|
| +25 | War Elephants |
| +22 | Cavalry |
| +18 | Camels |
| +16 | Ships & Saboteurs |
| +7 | Heroes & Kings |
| +1 | Eagle Warriors |
| +1 | Standard Buildings |

## Armor Classes (Vulnerability)

_Units with attack bonuses against these classes deal extra damage to this unit._

| Armor Class | Armor Value |
|-------------|-------------|
| Infantry | 0 |
| Spearmen | 0 |
| Leitis | 0 |

## Strengths & Weaknesses

**Strong vs:** Cavalry
**Weak vs:** Archer, Scorpion, Cataphract, infantry

## Ability

_Chieftains (generates gold when killing Villagers, trade units, and Monks, Vikings only)
Coiled Serpent Array (gain +0.5% HP (max +15%) per Spearman and White Feather Guard in 15 radius, Shu only)
Lamellar Armor (25% non-projectile damage reflection, Khitans only)
Tuntian (passively generate /min, Wei only)
Druzhina (deals 5 trample damage in 0.5 tile radius, Slavs only)
Wootz Steel (attack ignores armor, Dravidians only)_

## Technologies

| Stat | Technology (Effect) |
|------|---------------------|
| Attack | Forging (+1, +2 for the Khitans) |
| Attack | Iron Casting (+1, +2 for the Khitans) |
| Attack | Blast Furnace (+2) |
| Attack | Arson (+2 attack vs. Standard Buildings) |
| Attack | Chieftains (+5 against Cavalry and +4 against Camel Units, Vikings only) |
| Attack | Garland Wars (+4, Aztecs only) |
| Armor | Scale Mail Armor (melee=+1, pierce=+1, melee=+2, pierce=+2 for the Romans) |
| Armor | Chain Mail Armor (melee=+1, pierce=+1, melee=+2, pierce=+2 for the Romans) |
| Armor | Plate Mail Armor (melee=+1, pierce=+2) |
| Armor | Tower Shields (pierce=+2, Lithuanians only) |
| Speed | Squires (+10%) |
| Conversion | Devotion (+1 min, +1 max) |
| Conversion | Faith (+4 min, +4 max) |
| Conversion | Heresy (die upon getting converted) |
| Conversion | First Crusade (+4 min, +4 max, Sicilians only) |
| Creation | Conscription (+33%) |
| Creation | Perfusion (+100%, Goths only) |

## Civilization Bonuses

**Unique to civ:**

| Stat | Civ (Bonus) |
|------|-------------|
| HP | Vikings (+20%) |
| HP | Mapuche (+10/15 in the Castle/Imperial Age) |
| Attack | Bohemians (+25% bonus damage) |
| Attack | Burmese (+2/+3 in the Castle/Imperial Age) |
| Attack | Goths (+2/+3 against Standard Buildings in the Castle/Imperial Age) |
| Attack | Tatars (+20% damage {+50% instead of +25%} from higher elevation) |
| Armor | Malians (pierce=+2/+3 in the Castle/Imperial Age) |
| Armor | Teutons (melee=+1/+2 in the Castle/Imperial Age) |
| Speed | Celts (+15%/+20% faster in the Castle/Imperial Age) |
| Speed | Lithuanians (+10%) |
| Creation | Aztecs (+15%) |
| Creation | Khitans (+15%) |
| Other | Armenians (available in the Feudal Age) |

**Team bonuses:**

| Stat | Civ (Bonus) |
|------|-------------|
| Attack | Khitans (+2 vs. Archers) |
| Conversion | Teutons (+3 min, +1 max) |
| Creation | Goths (+20%, Barracks only) |
| LOS | Armenians (+2) |
| LOS | Mapuche (+2) |
