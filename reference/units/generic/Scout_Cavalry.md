# Scout Cavalry

**Type:** Standard  
**Available to:** All civs  
**Sources:** SiegeEngineers/aoe2techtree, Fandom wiki  
**Generated:** 2026-04-13

## Stats

| Stat | Castle | Elite |
|------|---------|-------|
| HP | 45.0 | — |
| Attack | 3.0 | — |
| Melee Armor | 0.0 | — |
| Pierce Armor | 2.0 | — |
| Speed | 1.2 | — |
| Range | 0.0 | — |
| Reload Time | 2.0 | — |
| Cost Food | 80.0 | — |
| Cost Wood | 0.0 | — |
| Cost Gold | 0.0 | — |
| Pop Space | 1.0 | — |

## Special Effects

- **pass_through_count:** 1.0

## DB Comparison

| Field | External | Our DB (Castle) | Match |
|-------|----------|-----------------|-------|
| HP | 45.0 | 45.0 | ✅ |
| Attack | ⚠️ | 3.0 | ⚠️ |
| Melee Armor | 0.0 | 0.0 | ✅ |
| Pierce Armor | 2.0 | 2.0 | ✅ |
| Speed | ⚠️ | 1.2 | ⚠️ |
| Range | ⚠️ | 0.0 | ⚠️ |
| Reload Time | 2.0 | 2.0 | ✅ |
| Cost Food | 150 | 80.0 | ❌ |
| Cost Wood | 0 | 0.0 | ✅ |
| Cost Gold | 50 | 0.0 | ❌ |

**⚠️ 2 mismatch(es) found — investigate.**

## Attack Bonuses

| Bonus | Armor Class |
|-------|-------------|
| +6 | Monks |
| -3 | Mounted Archers |

## Armor Classes (Vulnerability)

_Units with attack bonuses against these classes deal extra damage to this unit._

| Armor Class | Armor Value |
|-------------|-------------|
| Cavalry | 0 |
| Leitis | 0 |

## Strengths & Weaknesses

**Strong vs:** Skirmisher, Villager, siege weapon, Archer, Monk
**Weak vs:** Infantry, cavalry

## Ability

_Tuntian (passively generate /min, Wei only)
Lechitic Legacy (deals 33% trample damage in 0.5 tile radius, Poles only)
Wootz Steel (attack ignores armor, Dravidians only)_

## Technologies

| Stat | Technology (Effect) |
|------|---------------------|
| HP | Bloodlines (+20) |
| Attack | Forging (+1, +2 for the Khitans) |
| Attack | Iron Casting (+1, +2 for the Khitans) |
| Attack | Blast Furnace (+2) |
| Attack | Manipur Cavalry (+4 against Archers, Burmese only) |
| Attack | Farimba (+5, Malians only) |
| Armor | Scale Barding Armor (melee=+1, pierce=+1) |
| Armor | Chain Barding Armor (melee=+1, pierce=+1) |
| Armor | Plate Barding Armor (melee=+1, pierce=+2) |
| Armor | Silk Armor (melee=+1, pierce=+1, Tatars only) |
| Armor | Ming Guang Armor (melee=+4, Wei only) |
| Speed | Husbandry (+10%) |
| Conversion | Devotion (+1 min, +1 max) |
| Conversion | Faith (+4 min, +4 max) |
| Conversion | Heresy (die upon getting converted) |
| Conversion | First Crusade (+4 min, +4 max, Sicilians only) |
| Creation | Conscription (+33%) |
| Creation | Steppe Husbandry (+100%, Cumans only) |
| Creation | Chivalry (+40%, Franks only) |

## Civilization Bonuses

**Unique to civ:**

| Stat | Civ (Bonus) |
|------|-------------|
| HP | Franks (+20% starting in the Feudal Age) |
| HP | Mongols (+20%/+30% in the Castle/Imperial Age) |
| Attack | Bengalis (+2 attack vs. Skirmishers) |
| Attack | Gurjaras (+20%/+30%/+40% bonus damage in the Feudal/Castle/Imperial Age) |
| Attack | Tatars (+20% damage {+50% instead of +25%} from higher elevation) |
| Armor | Teutons (melee=+1/+2 in the Castle/Imperial Age) |
| Armor | Turks (pierce=+1) |
| Speed | Cumans (+5%/+10%/+15% in the Feudal/Castle/Imperial Age) |
| Creation | Khitans (+15%) |

**Team bonuses:**

| Stat | Civ (Bonus) |
|------|-------------|
| Attack | Hindustanis (+2 attack against Standard Buildings) |
| Attack | Poles (+1 vs. Archers) |
| Attack | Wei (+2 vs. Siege Weapons) |
| Conversion | Teutons (+3 min, +1 max) |
| Creation | Huns (+20%) |
| LOS | Mongols (+2) |
