# Demolition Ship

**Type:** Standard  
**Available to:** All civs  
**Sources:** SiegeEngineers/aoe2techtree, Fandom wiki  
**Generated:** 2026-04-13

## Stats

| Stat | Stats |
|------|---------|
| HP | 50 |
| Attack | 95 |
| Melee Armor | 1 |
| Pierce Armor | 0 |
| Speed | 1.52 |
| Range | 0 |
| Reload Time | 0 |
| Cost Food | 0 |
| Cost Wood | 45 |
| Cost Gold | 80 |
| Pop Space | 1 |

## Special Effects

None

## DB Comparison

_Naval units are not in the simulation database. Stats shown above are from the AoE2 wiki._

## Attack Bonuses

No bonus damage entries.

## Armor Classes (Vulnerability)

_Units with attack bonuses against these classes deal extra damage to this unit._

No non-base armor classes.

## Strengths & Weaknesses

**Strong vs:** Fire Ship, building, Cannon Galleon
**Weak vs:** Turtle Ship, tower, Castle

## Ability

_Red Cliffs Tactics (attacks deal an additional 5 damage per second for 5 seconds, Wu only)_

## Technologies

| Stat | Technology (Effect) |
|------|---------------------|
| Attack | Siege Engineers (+40% vs. All Buildings) |
| Attack | Cilician Fleet (+20% blast radius, Armenians only) |
| Armor | Careening (pierce=+1) |
| Armor | Dry Dock (pierce=+1) |
| Speed | Clinker Construction (+10%) |
| Speed | Carvel Hull (+10%) |
| Conversion | Devotion (+1 min, +1 max) |
| Conversion | Faith (+4 min, +4 max) |
| Conversion | Heresy (die upon getting converted) |
| Conversion | First Crusade (+4 min, +4 max, Sicilians only) |
| Creation | Shipwright (+54%) |

## Civilization Bonuses

**Unique to civ:**

| Stat | Civ (Bonus) |
|------|-------------|
| Speed | Berbers (+10%) |
| Creation | Aztecs (+15%) |
| Creation | Persians (+15%/+20% in the Castle/Imperial Age) |

**Team bonuses:**

| Stat | Civ (Bonus) |
|------|-------------|
| Conversion | Teutons (+3 min, +1 max) |
