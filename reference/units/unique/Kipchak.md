# Kipchak

**Type:** Unique  
**Available to:** Cumans  
**Sources:** SiegeEngineers/aoe2techtree, Fandom wiki  
**Generated:** 2026-04-13

## Stats

| Stat | Castle | Imperial |
|------|---------|-------|
| HP | 40.0 | 45.0 |
| Attack | 4.0 | 5.0 |
| Melee Armor | 0.0 | 0.0 |
| Pierce Armor | 0.0 | 0.0 |
| Speed | 1.4 | 1.4 |
| Range | 4.0 | 4.0 |
| Reload Time | 2.2 | 2.2 |
| Cost Food | 0.0 | 0.0 |
| Cost Wood | 60.0 | 60.0 |
| Cost Gold | 35.0 | 35.0 |
| Pop Space | 1.0 | 1.0 |

## Special Effects

- **pass_through_count:** 1.0

## DB Comparison

| Field | External | Our DB (Castle) | Match |
|-------|----------|-----------------|-------|
| HP | 40 | 40.0 | ✅ |
| Attack | 4 | 4.0 | ✅ |
| Melee Armor | 0 | 0.0 | ✅ |
| Pierce Armor | 0 | 0.0 | ✅ |
| Speed | 1.4 | 1.4 | ✅ |
| Range | 4 | 4.0 | ✅ |
| Reload Time | 2.2 | 2.2 | ✅ |
| Cost Food | 0 | 0.0 | ✅ |
| Cost Wood | 60 | 60.0 | ✅ |
| Cost Gold | 35 | 35.0 | ✅ |

## Attack Bonuses

| Bonus | Armor Class |
|-------|-------------|
| -3 | Mounted Archers |
| +1 | Spearmen |

## Armor Classes (Vulnerability)

_Units with attack bonuses against these classes deal extra damage to this unit._

| Armor Class | Armor Value |
|-------------|-------------|
| Cavalry | 0 |
| Archers | 0 |
| Unique Units | 0 |
| Cavalry Archers | 0 |
| Leitis | 0 |

## Strengths & Weaknesses

**Strong vs:** ram, Monk
**Weak vs:** Archer, camel unit

## Ability

_Tuntian (Passively generate /min, Wei only)_

## Technologies

| Stat | Technology (Effect) |
|------|---------------------|
| HP | Bloodlines (+20) |
| HP | Sipahi (+20, Turks only) |
| Attack | Fletching (+1) |
| Attack | Bodkin Arrow (+1) |
| Attack | Bracer (+1, allies only) |
| Attack | Chemistry (+1) |
| Attack | Parthian Tactics (+2 vs Spearmen) |
| Attack | Recurve Bow (+1, Magyars only) |
| Armor | Padded Archer Armor (+1/+1) |
| Armor | Leather Archer Armor (+1/+1) |
| Armor | Ring Archer Armor (+1/+2) |
| Armor | Parthian Tactics (+1/+2) |
| Armor | Silk Armor (+1/+1, Tatars only) |
| Speed | Husbandry (+10%, allies only) |
| Conversion | Devotion (+1 min, +1 max) |
| Conversion | Faith (+4 min, +4 max) |
| Conversion | Heresy (die upon getting converted) |
| Conversion | First Crusade (+4 min, +4 max, Sicilians only) |
| Creation | Conscription (+33%) |
| Creation | Kasbah (+25%, Berbers and allies only) |
| Other | Aznauri Cavalry (-20% population space, Georgians only) |

## Civilization Bonuses

**Unique to civ:**

| Stat | Civ (Bonus) |
|------|-------------|
| HP | Franks (+20%) |
| HP | Vietnamese (+20%) |
| Attack | Japanese (+2 vs. Archers, excluding Skirmishers) |
| Attack | Tatars (+20% {+50% instead of +25%} from elevation) |
| Speed | Cumans (+10%/15% in the Castle/Imperial Age) |
| Creation | Aztecs (+15%) |

**Team bonuses:**

| Stat | Civ (Bonus) |
|------|-------------|
| Conversion | Teutons (+3 min, +1 max) |
| Creation | Magyars (+25%) |
| LOS | Tatars (+2) |
