# Konnik

**Type:** Unique  
**Available to:** Bulgarians  
**Sources:** SiegeEngineers/aoe2techtree, Fandom wiki  
**Generated:** 2026-04-13

## Stats

| Stat | Castle | Imperial |
|------|---------|-------|
| HP | 100.0 | 120.0 |
| Attack | 12.0 | 14.0 |
| Melee Armor | 2.0 | 2.0 |
| Pierce Armor | 2.0 | 2.0 |
| Speed | 1.35 | 1.35 |
| Range | 0.0 | 0.0 |
| Reload Time | 2.4 | 2.4 |
| Cost Food | 60.0 | 60.0 |
| Cost Wood | 0.0 | 0.0 |
| Cost Gold | 70.0 | 70.0 |
| Pop Space | 1.0 | 1.0 |

## Special Effects

- **pass_through_count:** 1.0
- **dismount_hp:** 45.0
- **dismount_attack:** 12.0
- **dismount_melee_armor:** 1.0
- **dismount_pierce_armor:** 1.0
- **dismount_attack_speed:** 2.4
- **dismount_movement_speed:** 0.9

## DB Comparison

| Field | External | Our DB (Castle) | Match |
|-------|----------|-----------------|-------|
| HP | 100 | 100.0 | ✅ |
| Attack | 12 | 12.0 | ✅ |
| Melee Armor | 2 | 2.0 | ✅ |
| Pierce Armor | 2 | 2.0 | ✅ |
| Speed | 1.35 | 1.35 | ✅ |
| Range | 0 | 0.0 | ✅ |
| Reload Time | 2.4 | 2.4 | ✅ |
| Cost Food | 60 | 60.0 | ✅ |
| Cost Wood | 0 | 0.0 | ✅ |
| Cost Gold | 70 | 70.0 | ✅ |

## Attack Bonuses

| Bonus | Armor Class |
|-------|-------------|
| -3 | Mounted Archers |

## Armor Classes (Vulnerability)

_Units with attack bonuses against these classes deal extra damage to this unit._

| Armor Class | Armor Value |
|-------------|-------------|
| Cavalry | 0 |
| Unique Units | 0 |
| Leitis | 0 |

## Strengths & Weaknesses

**Strong vs:** Infantry, siege weapon, light cavalry
**Weak vs:** Archer, Pikemen, Monk, Camel Rider, siege weapon, Boyar, Kamayuk, Berserk, Genoese Crossbowmen, Teutonic Knight, Mameluke, War Elephant, Battle Elephant, Samurai, Cataphract, Flemish Militia, Composite Bowmen

## Ability

_Turn into a(n) (Elite) Dismounted Konnik upon death_

## Technologies

| Stat | Technology (Effect) |
|------|---------------------|
| HP | Bloodlines (+20, Mounted) |
| Attack | Forging (+1) |
| Attack | Iron Casting (+1) |
| Attack | Blast Furnace (+2) |
| Attack | Arson (+2 against Standard Buildings, Dismounted) |
| Armor | Scale Barding Armor (melee=+1, pierce=+1, Mounted) |
| Armor | Scale Mail Armor (melee=+1, pierce=+1, Dismounted) |
| Armor | Chain Barding Armor (melee=+1, pierce=+1, Mounted) |
| Armor | Chain Mail Armor (melee=+1, pierce=+1, Dismounted) |
| Armor | Plate Barding Armor (melee=+1, pierce=+2, Mounted) |
| Armor | Plate Mail Armor (melee=+1, pierce=+2, Dismounted) |
| Speed | Husbandry (+10%, Mounted) |
| Speed | Squires (+10%, Dismounted) |
| Conversion | Devotion (+1 min, +1 max) |
| Conversion | Heresy (die upon getting converted) |
| Creation | Conscription (+33%) |
| Creation | Kasbah (+25%, for Castle-trained Konniks with a Berber ally only) |

## Civilization Bonuses

**Team bonuses:**

| Stat | Civ (Bonus) |
|------|-------------|
| Attack | Khitans (+2 against Archers, Dismounted) |
| Attack | Wei (+2 against Siege Weapons, Mounted) |
| Conversion | Teutons (+3 min, +1 max) |
| LOS | Armenians (+2, Dismounted) |
