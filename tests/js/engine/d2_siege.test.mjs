...
test("D2 ships blastDebris ON and the other three OFF", () => {
    // The merge decision: S2c is a dat fact (one shot = stone + 9 fragments,
    // exact 9.000 in every recording) and the one D2 rule the board endorses
    // (-0.52 opponent / -1.03 onager HP error, no row worse by >0.2). The
    // rest stay off with named blockers: boltCorridor gated by the opponent-
    // cadence defect, blastZeroPoint by two re-record tapes, projectileArc by
    // an unmeasured hit-rate interaction. See constants.js.
    assert.equal(D2.blastDebris, true, "the endorsed dat-fact rule ships on");
    for (const k of ["boltCorridor", "blastZeroPoint", "projectileArc"]) {
        assert.equal(D2[k], false, `D2.${k} ships off until its blocker clears`);
    }
});
