...
# Mounted on the analyzer's Flask app; all routes live under /replay/...
...
@replay_bp.route("/replay/api/upload", methods=["POST"])
def upload_replay():
...
@replay_bp.route("/replay/api/matches", methods=["GET"])
def get_matches():
...
@replay_bp.route("/replay/api/matches/<player_name>", methods=["GET"])
...
@replay_bp.route("/replay/api/players", methods=["GET"])
def search_players():
...
@replay_bp.route("/replay/api/player/<int:profile_id>/matches", methods=["GET"])
...
@replay_bp.route("/replay/api/load-match", methods=["POST"])
def load_match():
...
def make_clip():
    ...
    return jsonify({
        "clip_url": f"{base}/static/replay/clips/{out_name}",
        # Use the ?match=&profile= deep-link params the replay frontend reads
        # (app.js init()), on the /replay page so the analyzer nav stays visible
        # and the shared "Watch in UI" link auto-loads the replay.
        "view_url": f"{base}/replay?match={match_id}&profile={profile_id}",
    })
