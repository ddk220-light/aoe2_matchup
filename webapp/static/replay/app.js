...
class App {
  ...
  async searchPlayers() {
    ...
    try {
      const response = await fetch(
        `/replay/api/players?search=${encodeURIComponent(query)}`,
      );
      if (!response.ok) {
  ...
  async fetchMatchesForPlayer(profileId) {
    const response = await fetch(
      `/replay/api/player/${profileId}/matches?limit=10`,
    );
    if (!response.ok) {
      const error = await response.json().catch(() => ({}));
  ...
  async loadMatch(match) {
    ...
    try {
      const response = await fetch("/replay/api/load-match", {
        method: "POST",
  ...
  async generateClip() {
    ...
    try {
      ...
      const resp = await fetch(`/replay/api/clip?${params.toString()}`);
      if (!resp.ok) {
