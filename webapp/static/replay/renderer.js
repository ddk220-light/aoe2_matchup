...
class Renderer {
  ...
  async loadSprites() {
    try {
      ...
      const response = await fetch("/static/replay/assets/sprites/sprites.json");
  ...
  loadSpriteImage(name, file) {
    return new Promise((resolve) => {
      ...
      img.src = `/static/replay/assets/sprites/${file}`;
    });
  }
  ...
  // Lazily load a civ emblem (e.g. "Mongols" -> /static/replay/assets/civs/mongols.png) into
  ...
  loadCivEmblem(civ) {
    ...
    img.src = `/static/replay/assets/civs/${key}.png`;
  }
  ...
  render(state) {
    ...
    // Trebuchet shots: arcing flaming balls from siege to the building.
    for (const tp of state.trebProjectiles || []) {
      this.drawTrebProjectiles(tp);
    }
  ...
  drawTrebProjectiles(tp) {
    ...
    const to = this.gameToCanvas(tp.toX, tp.toY);
